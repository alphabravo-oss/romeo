...
function mockFetch(returnBody: unknown = { data: {} }) {
  const fn = vi.fn(async (url: string, _init?: RequestInit) => {
    const textResponse = url.endsWith(".csv") || url.endsWith("/content");
    return {
      headers: new Headers({
        "content-type": textResponse ? "text/plain" : "application/json",
      }),
      ok: true,
      json: async () => returnBody,
      text: async () =>
        textResponse ? "body-text" : JSON.stringify(returnBody),
...
describe("admin-client — data exports", () => {
  ...
  it("createDataExportPackage POSTs the packages route", async () => {
    ...
    const created = await createDataExportPackage({
      scope: "org",
      includeContent: true,
    });
    const call = lastCall(fn);
  ...
  it("deleteDataExportPackage DELETEs the package with the confirm guard (id url-encoded)", async () => {
    const fn = mockFetch({ data: {} });
    await deleteDataExportPackage({
      packageId: "pkg 1",
      confirmPackageId: "pkg 1",
    });
    const call = lastCall(fn);
  ...
  it("downloadDataExportPackageContent fetches the content route and returns text", async () => {
    ...
    expect(lastCall(fn).url).toBe(
      "/api/v1/governance/data-exports/packages/pkg1/content",
    );
