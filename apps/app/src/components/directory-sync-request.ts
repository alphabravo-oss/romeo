 ...
 */
function parseGroupMemberships(text: string): {
  value?: DirectorySyncGroupInventory[];
  error?: string;
} {
  const lines = text
    .split("\n")
  ...
  for (const line of lines) {
    ...
    if (!groupId)
      return { error: `Group line is missing a group id: "${line}".` };
    if (groupId.length > 120)
      return { error: `Group id is too long (max 120): "${groupId}".` };
    ...
    for (const userId of presentUserIds) {
      if (userId.length > 120)
        return {
          error: `User id is too long (max 120) in group "${groupId}".`,
        };
    }
...
export function buildDirectorySyncRequest(
  ...
  const removalCap = parseCap(
    form.maxMembershipRemovals,
    10_000,
    "Max membership removals",
  );
  ...
  const request: DirectorySyncRequest = {
    ...
    ...(opts.apply
      ? { confirmApply: "apply-directory-sync", dryRun: false }
      : { dryRun: true }),
    ...(reason ? { reason } : {}),
    ...(emails.length ? { presentUserEmails: emails } : {}),
    ...(groups.value && groups.value.length
      ? { groupMemberships: groups.value }
      : {}),
    ...(form.disableMissingUsers ? { disableMissingUsers: true } : {}),
    ...(form.removeMissingGroupMembers
    ...
    ...(removalCap.value !== undefined
      ? { maxMembershipRemovals: removalCap.value }
      : {}),
  };
