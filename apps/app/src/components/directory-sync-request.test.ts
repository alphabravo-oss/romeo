...
describe("buildDirectorySyncRequest", () => {
  it("previews with dryRun and no confirmApply", () => {
    const res = buildDirectorySyncRequest(defaultDirectorySyncForm, {
      apply: false,
    });
    expect(res).toEqual({
      ok: true,
  ...
  it("parses present emails and rejects malformed ones", () => {
    ...
    expect(ok.ok && ok.request.presentUserEmails).toEqual([
      "a@b.com",
      "c@d.com",
    ]);
  ...
  it("rejects a group line with no group id", () => {
    expect(
      buildDirectorySyncRequest(
        { ...defaultDirectorySyncForm, groupMemberships: ": u1, u2" },
        { apply: false },
      ),
    ).toEqual({
      ok: false,
      error: 'Group line is missing a group id: ": u1, u2".',
    });
  });
  ...
  it("rejects out-of-range caps", () => {
    expect(
      buildDirectorySyncRequest(
        { ...defaultDirectorySyncForm, maxUserDisables: "2000" },
        { apply: false },
      ),
    ).toEqual({
      ok: false,
      error: "Max user disables must be a whole number between 0 and 1000.",
    });
    expect(
      buildDirectorySyncRequest(
        { ...defaultDirectorySyncForm, maxMembershipRemovals: "-1" },
        { apply: false },
      ),
    ).toEqual({
      ok: false,
      error:
        "Max membership removals must be a whole number between 0 and 10000.",
    });
  });
});
