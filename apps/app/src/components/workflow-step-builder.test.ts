...
describe("buildWorkflowSteps", () => {
  it("rejects an empty step list", () => {
    expect(buildWorkflowSteps([])).toEqual({
      ok: false,
  ...
  it("builds an agent_run step with only its variant fields", () => {
    const res = buildWorkflowSteps([
      draft("agent_run", { name: "Run", agentId: "a1" }),
    ]);
    expect(res).toEqual({
      ok: true,
      steps: [{ id: "step_1", name: "Run", type: "agent_run", agentId: "a1" }],
    });
  });
  ...
  it("parses agent_room ids and enforces 2..5 unique", () => {
    expect(
      buildWorkflowSteps([
        draft("agent_room", { name: "Room", agentIds: "a1" }),
      ]),
    ).toEqual({
      ok: false,
  ...
  it("validates tool_approval input keys and includes optional fields", () => {
    expect(
      buildWorkflowSteps([
        draft("tool_approval", { name: "T", inputKeys: "good, bad key" }),
      ]),
    ...
    expect(
      buildWorkflowSteps([
        draft("agent_handoff", {
          name: "H",
          agentId: "a1",
          handoffFromStepId: "nope",
        }),
      ]),
    ).toEqual({
      ok: false,
      error: "Step 1: handoff source must be an earlier step.",
    });
