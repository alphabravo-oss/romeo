...
import type { WorkflowStep, WorkflowStepType } from "../features/workflows";
...
export function buildWorkflowSteps(drafts: StepDraft[]): BuildStepsResult {
  ...
  for (let i = 0; i < drafts.length; i += 1) {
    ...
    if (name.length > 120)
      return { ok: false, error: `${label}: name is too long (max 120).` };
    ...
    switch (draft.type) {
      case "agent_run": {
        ...
        if (!agentId)
          return { ok: false, error: `${label}: agent id is required.` };
      ...
      case "agent_handoff": {
        ...
        if (from && !/^step_[1-9][0-9]*$/.test(from)) {
          return {
            ok: false,
            error: `${label}: handoff source must be an earlier step.`,
          };
        }
      ...
      case "agent_room": {
        ...
        if (agentIds.length < 2)
          return { ok: false, error: `${label}: add at least 2 agent ids.` };
        if (agentIds.length > 5)
          return { ok: false, error: `${label}: at most 5 agent ids.` };
      ...
      case "tool_approval": {
        ...
        for (const key of inputKeys) {
          if (!INPUT_KEY_RE.test(key))
            return {
              ok: false,
              error: `${label}: invalid input key "${key}".`,
            };
        }
      ...
      case "browser_task": {
        ...
        if (!targetUrl)
          return { ok: false, error: `${label}: target URL is required.` };
        try {
          new URL(targetUrl);
        } catch {
          return {
            ok: false,
            error: `${label}: target URL is not a valid URL.`,
          };
        }
