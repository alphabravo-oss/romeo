...
const analyticsAuthzRequiredChecks = [
...
function analyticsAuthzLiveEvidence(overrides: Record<string, unknown> = {}) {
  return {
    schemaVersion: "romeo.analytics-authz-live-evidence.v1",
    generatedAt: "2026-07-06T22:05:00.000Z",
    status: "passed",
    mode: "live",
    deployment: "kubernetes",
    checks: [...analyticsAuthzRequiredChecks],
    subjects: {
      adminSubjectCount: 1,
...
const voiceProviderLiveRequiredChecks = [
...
function voiceProviderLiveEvidence(overrides: Record<string, unknown> = {}) {
  return {
    ...
    mode: "live",
    deployment: "kubernetes",
    checks: [...voiceProviderLiveRequiredChecks],
    provider: {
...
const notificationAdapterLiveRequiredChecks = [
...
function notificationAdapterLiveEvidence(
  ...
  return {
    ...
    mode: "live",
    deployment: "kubernetes",
    checks: [...notificationAdapterLiveRequiredChecks],
    delivery: {
...
const billingOperationsRequiredChecks = [
...
function billingOperationsEvidence(overrides: Record<string, unknown> = {}) {
  return {
    ...
    mode: "live",
    deployment: "kubernetes",
    checks: [...billingOperationsRequiredChecks],
    cadence: {
      windowMinutes: 120,
      expectedRunCount: 2,
      observedRunCount: 2,
...
const toolDispatchLiveRequiredChecks = [
...
function toolDispatchLiveEvidence(overrides: Record<string, unknown> = {}) {
  return {
    ...
    deployment: "kubernetes",
    generatedAt: "2026-01-01T00:00:00.000Z",
    checks: [...toolDispatchLiveRequiredChecks],
    operations: {
...
describe("Romeo API thin slice", () => {
  it("serves health and the Milestone 1 OpenAPI document", async () => {
    ...
    expect(
      spec.components.schemas.TenantPurgeEvidencePostureReport,
    ).toBeDefined();
    expect(Object.keys(spec.paths)).toContain("/admin/identity/live-posture");
    expect(spec.components.schemas.IdentityLivePostureReport).toBeDefined();
    expect(Object.keys(spec.paths)).toContain("/admin/analytics/authz-posture");
    expect(spec.components.schemas.AnalyticsAuthzPostureReport).toBeDefined();
    ...
    expect(Object.keys(spec.paths)).toContain("/admin/support-bundle/posture");
    expect(Object.keys(spec.paths)).toContain("/admin/ci-governance/posture");
    expect(spec.components.schemas.ReleaseReadbackPostureReport).toBeDefined();
    expect(spec.components.schemas.ReleaseSecurityPostureReport).toBeDefined();
    expect(spec.components.schemas.SupportBundlePostureReport).toBeDefined();
    expect(Object.keys(spec.paths)).toContain("/admin/target-quality/posture");
    expect(spec.components.schemas.TargetQualityPostureReport).toBeDefined();
    expect(Object.keys(spec.paths)).toContain("/admin/alert-firing/posture");
    expect(spec.components.schemas.AlertFiringPostureReport).toBeDefined();
    expect(Object.keys(spec.paths)).toContain(
      "/admin/migrations/drill-posture",
    );
    expect(spec.components.schemas.MigrationDrillPostureReport).toBeDefined();
    expect(Object.keys(spec.paths)).toContain(
      "/admin/network/partition-posture",
    );
    expect(spec.components.schemas.NetworkPartitionPostureReport).toBeDefined();
    expect(Object.keys(spec.paths)).toContain(
      "/admin/secret-rotation/drill-posture",
    );
    expect(
      spec.components.schemas.SecretRotationDrillPostureReport,
    ).toBeDefined();
    ...
    expect(Object.keys(spec.paths)).toContain("/admin/tool-dispatch/posture");
    expect(spec.components.schemas.ToolDispatchPostureReport).toBeDefined();
    expect(Object.keys(spec.paths)).toContain("/admin/kubernetes/posture");
    expect(spec.components.schemas.KubernetesPostureReport).toBeDefined();
    expect(
    ...
    expect(
      spec.paths["/admin/tool-dispatch/posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/ToolDispatchPostureReport");
    expect(
      spec.paths["/admin/kubernetes/posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/KubernetesPostureReport");
    expect(
      spec.paths["/admin/release-readback/posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/ReleaseReadbackPostureReport");
    expect(
      spec.paths["/admin/release-security/posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/ReleaseSecurityPostureReport");
    expect(
      spec.paths["/admin/support-bundle/posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/SupportBundlePostureReport");
    expect(
      spec.paths["/admin/ci-governance/posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/CiGovernancePostureReport");
    expect(
      spec.paths["/admin/billing/operations-posture"].get.responses[200]
        .content["application/json"].schema.properties.data.$ref,
    ).toBe("#/components/schemas/BillingOperationsPostureReport");
    expect(
      spec.paths["/admin/providers/outage-posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/ProviderOutagePostureReport");
    expect(
      spec.paths["/admin/identity/live-posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/IdentityLivePostureReport");
    expect(
      spec.paths["/admin/analytics/authz-posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/AnalyticsAuthzPostureReport");
    expect(
      spec.paths["/admin/migrations/drill-posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/MigrationDrillPostureReport");
    expect(
      spec.paths["/admin/network/partition-posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/NetworkPartitionPostureReport");
    expect(
      spec.paths["/admin/secret-rotation/drill-posture"].get.responses[200]
        .content["application/json"].schema.properties.data.$ref,
    ).toBe("#/components/schemas/SecretRotationDrillPostureReport");
    expect(
      spec.paths["/admin/alert-firing/posture"].get.responses[200].content[
        "application/json"
      ].schema.properties.data.$ref,
    ).toBe("#/components/schemas/AlertFiringPostureReport");
    expect(
    ...
    expect(
      spec.components.schemas.BillingOperationsPostureReport,
    ).toBeDefined();
    expect(spec.components.schemas.ProviderOutagePostureReport).toBeDefined();
    expect(spec.components.schemas.MigrationDrillPostureReport).toBeDefined();
    expect(spec.components.schemas.NetworkPartitionPostureReport).toBeDefined();
    expect(
      spec.components.schemas.SecretRotationDrillPostureReport,
  ...
  it("reports sanitized support bundle posture without raw bundle content", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-support-posture-"));
    const bundlePath = join(directory, "support-bundle.json");
    const redactionPath = join(directory, "support-bundle-redaction.json");
    writeFileSync(
      bundlePath,
      JSON.stringify({
        ...
        package: {
          ...
          rawPackageJson: "RAW_SUPPORT_PACKAGE_BODY",
        },
        runtime: { node: "v24.0.0", platform: "linux", arch: "arm64" },
        configuration: {
          safeEnums: {
            REPOSITORY_DRIVER: "postgres",
            UNKNOWN_RAW_SUPPORT_ENUM: "configured_unrecognized",
          },
          safeNumbers: { POSTGRES_POOL_MAX: 20 },
          configuredSecrets: {
            MANAGED_SECRET_ENCRYPTION_KEY: true,
            RAW_SUPPORT_SECRET_NAME: true,
          },
          urlHosts: { APP_ORIGIN: "romeo.example.com" },
          rawEnvironment: { DATABASE_URL: "RAW_SUPPORT_DATABASE_URL" },
        },
        deployment: [
          {
            file: join(directory, "deploy-compose.yml"),
            body: "RAW_SUPPORT_DEPLOY",
          },
          { file: join(directory, "values.yaml"), body: "RAW_SUPPORT_VALUES" },
        ],
        ...
        complianceEvidence: {
          accessReview: {
            status: "present",
            count: 2,
            body: "RAW_ACCESS_REVIEW_BODY",
          },
        },
        dataRights: {
          coverageApiPath: "/api/v1/data-rights/coverage",
          exportPreviewApiPath: "/api/v1/governance/data-export/preview",
          exportExecuteApiPath: "/api/v1/governance/data-export",
          deletionPreviewApiPath: "/api/v1/governance/data-deletion/preview",
          deletionExecuteApiPath: "/api/v1/governance/data-deletion",
          ...
          retentionEvidence: {
            ...
            backupLocation: "RAW_BACKUP_LOCATION",
          },
          externalRetentionControls: ["s3-lifecycle", "postgres-backup"],
        },
        logs: [
          {
            name: "api.log",
        ...
        providerPayloads: [{ body: "RAW_PROVIDER_PAYLOAD" }],
        prompts: ["RAW_PROMPT_CONTENT"],
        objectStoreKeys: ["RAW_OBJECT_STORE_KEY"],
        vectors: ["RAW_VECTOR_VALUE"],
        redaction: {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        supportBundle: {
          schemaVersion: "romeo.support-bundle.v1",
          evidenceCount: 1,
          accessReviewEvidenceCount: 2,
          logCount: 1,
          migrationCount: 1,
          configuredSecretKeys: [
            "MANAGED_SECRET_ENCRYPTION_KEY",
            "RAW_SUPPORT_SECRET_NAME",
          ],
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        ...
        SUPPORT_BUNDLE_REDACTION_EVIDENCE_PATH: redactionPath,
      }),
    });

    const response = await api.request("/api/v1/admin/support-bundle/posture");
    const body = await response.json();
    const serialized = JSON.stringify(body);

    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.support-bundle-posture.v1");
    ...
    expect(body.data.bundle.configuration).toMatchObject({
      safeEnumCount: 2,
      configuredSafeEnumCount: 2,
      unrecognizedSafeEnumCount: 1,
      safeNumberCount: 1,
      configuredSecretCount: 2,
      urlHostConfiguredCount: 1,
    });
    expect(body.data.bundle.migrations.greenfieldBaselineOnly).toBe(true);
    expect(body.data.redactionEvidence.checks).toMatchObject({
      requiredPresent: 7,
      missingRequired: [],
    });
    expect(body.data.redaction).toMatchObject({
      evidenceFileBodiesReturned: false,
    ...
    expect(serialized).not.toContain("RAW_SUPPORT");
    expect(serialized).not.toContain("RAW_EVIDENCE_BODY");
    expect(serialized).not.toContain("RAW_ACCESS_REVIEW_BODY");
    expect(serialized).not.toContain("RAW_LOG_BODY");
    expect(serialized).not.toContain("RAW_BACKUP_LOCATION");
    expect(serialized).not.toContain("RAW_OBJECT_STORE_KEY");
    expect(serialized).not.toContain("RAW_VECTOR_VALUE");
    expect(serialized).not.toContain("RAW_PROMPT_CONTENT");
  });
  ...
  it("reports tool-dispatch live posture without leaking operation evidence details", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-tool-dispatch-"));
    const evidencePath = join(directory, "tool-dispatch-live-evidence.json");
    const rawHost = "https://internal-tool.secret.example/operate";
    const rawPayload = "RAW_TOOL_DISPATCH_PAYLOAD_SENTINEL";
    const rawResponse = "RAW_TOOL_DISPATCH_RESPONSE_SENTINEL";
    const rawObjectKey = "tool-dispatch-payloads/v1/org/job/payload";
    const rawSecretRef = "vault://romeo/tools/service-token";
    const rawMcpArguments = "RAW_TOOL_DISPATCH_MCP_ARGUMENTS_SENTINEL";
    const rawMcpOutput = "RAW_TOOL_DISPATCH_MCP_OUTPUT_SENTINEL";
    writeFileSync(
      evidencePath,
      JSON.stringify(
        toolDispatchLiveEvidence({
          rawHost,
          rawPayload,
          rawResponse,
          rawObjectKey,
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        ...
        TOOL_DISPATCH_LIVE_EVIDENCE_PATH: evidencePath,
    ...
    const response = await api.request("/api/v1/admin/tool-dispatch/posture");
    const body = await response.json();
    const serialized = JSON.stringify(body);
    ...
    expect(body.data).toMatchObject({
      ...
      deployment: {
        ...
        liveEvidencePathConfigured: true,
        networkPolicyConfigured: true,
        payloadEncryptionKeyConfigured: true,
        payloadStoreConfigured: true,
        workerEnabled: true,
      },
      backend: {
    ...
    expect(body.data.queue.total).toBe(0);
    expect(body.data.payloadStorage.managedEncryptedObjectStore).toBe(0);
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain("internal-tool.secret.example");
    expect(serialized).not.toContain(rawHost);
    expect(serialized).not.toContain(rawPayload);
    expect(serialized).not.toContain(rawResponse);
    expect(serialized).not.toContain(rawObjectKey);
    expect(serialized).not.toContain(rawSecretRef);
    expect(serialized).not.toContain(rawMcpArguments);
    expect(serialized).not.toContain(rawMcpOutput);
  });

  it("requires structured MCP proof before tool-dispatch live posture is ready", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-tool-dispatch-mcp-"));
    const evidencePath = join(directory, "tool-dispatch-live-evidence.json");
    writeFileSync(
      evidencePath,
      JSON.stringify(toolDispatchLiveEvidence({ mcp: {} })),
    );
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        TOOL_OPERATION_EXECUTION_DRIVER: "http-fetch",
    ...
    expect(response.status).toBe(200);
    expect(body.data.status).toBe("attention_required");
    expect(body.data.liveEvidence).toMatchObject({
      status: "failed",
      mcp: {
      ...
      failureCodes: ["tool_dispatch_live_mcp_invalid"],
    });
    expect(body.data.warnings).toContain("tool_dispatch_live_evidence_invalid");
  });

  it("rejects tool-dispatch live posture evidence that fails GA-aligned worker-boundary proofs", async () => {
    const directory = mkdtempSync(
      join(tmpdir(), "romeo-tool-dispatch-negative-"),
    );
    const baseEvidence = toolDispatchLiveEvidence();
    const cases = [
      {
        name: "deployment",
        evidence: toolDispatchLiveEvidence({ deployment: "compose" }),
        failureCode: "tool_dispatch_live_deployment_invalid",
      },
      {
        name: "egress",
        evidence: toolDispatchLiveEvidence({
          egress: {
            ...baseEvidence.egress,
            workerCniOrNetworkPolicyEnforced: false,
          },
        }),
        failureCode: "tool_dispatch_live_egress_invalid",
      },
      {
        name: "readback",
        evidence: toolDispatchLiveEvidence({
          readback: {
            ...baseEvidence.readback,
            dispatchReadbackVerified: false,
          },
        }),
        failureCode: "tool_dispatch_live_readback_invalid",
      },
    ];
  ...
  it("reports voice provider live posture without leaking voice evidence details", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-voice-provider-"));
    const evidencePath = join(directory, "voice-provider-live-evidence.json");
    const rawSpeechText = "RAW_VOICE_PROVIDER_SPEECH_TEXT_SENTINEL";
    const rawTranscriptText = "RAW_VOICE_PROVIDER_TRANSCRIPT_SENTINEL";
    const rawProviderEndpoint = "https://voice-provider.internal.example/v1";
    const rawProviderResponse = "RAW_VOICE_PROVIDER_RESPONSE_SENTINEL";
    const rawObjectKey = "voice/org_default/artifact-id/audio.wav";
    const rawAudio = "RAW_VOICE_PROVIDER_AUDIO_SENTINEL";
    writeFileSync(
      evidencePath,
      JSON.stringify({
        schemaVersion: "romeo.voice-provider-live-evidence.v1",
        ...
        secretValue: "RAW_VOICE_PROVIDER_SECRET_SENTINEL",
      }),
      "utf8",
    );
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        ...
        VOICE_OPENAI_API_KEY: "voice-live-test-key",
        VOICE_PROVIDER_LIVE_EVIDENCE_PATH: evidencePath,
      }),
    });
    ...
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain(rawSpeechText);
    expect(serialized).not.toContain(rawTranscriptText);
    expect(serialized).not.toContain(rawProviderEndpoint);
    expect(serialized).not.toContain(rawProviderResponse);
    expect(serialized).not.toContain(rawObjectKey);
    expect(serialized).not.toContain(rawAudio);
    expect(serialized).not.toContain("RAW_VOICE_PROVIDER_SECRET_SENTINEL");
  });

  it("rejects voice provider live posture evidence that fails GA-aligned required proofs", async () => {
    const directory = mkdtempSync(
      join(tmpdir(), "romeo-voice-provider-negative-"),
    );
    const baseEvidence = voiceProviderLiveEvidence();
    const cases = [
      {
        name: "deployment",
        evidence: voiceProviderLiveEvidence({ deployment: "developer-laptop" }),
        failureCode: "voice_provider_live_deployment_invalid",
        warning: "voice_provider_live_deployment_invalid",
      },
      {
        name: "catalog-sync",
        evidence: voiceProviderLiveEvidence({
          provider: {
            ...baseEvidence.provider,
            catalogSyncCount: 0,
          },
        }),
        failureCode: "voice_provider_live_tts_invalid",
        warning: "voice_provider_live_tts_missing",
      },
      {
        name: "streaming-consent",
        evidence: voiceProviderLiveEvidence({
          streamingConsent: {
        ...
        failureCode: "voice_provider_live_streaming_consent_invalid",
        warning: "voice_provider_live_streaming_consent_missing",
      },
      {
        ...
        failureCode: "voice_provider_live_failure_redaction_invalid",
        warning: "voice_provider_live_failure_redaction_missing",
      },
    ];

    for (const item of cases) {
      const evidencePath = join(directory, `${item.name}.json`);
      writeFileSync(evidencePath, JSON.stringify(item.evidence), "utf8");
      const api = createRomeoApi(new InMemoryRomeoRepository(), {
        env: readEnv({
          VOICE_PROVIDER_DRIVER: "openai-compatible",
          VOICE_OPENAI_API_KEY: "voice-live-negative-key",
          VOICE_PROVIDER_LIVE_EVIDENCE_PATH: evidencePath,
        }),
      });
      ...
      expect(body.data.evidence.failureCodes).toContain(item.failureCode);
      expect(body.data.warnings).toContain(item.warning);
    }
  });

  it("reports notification adapter live posture without leaking adapter evidence details", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-notification-live-"));
    const evidencePath = join(
      directory,
      "notification-adapter-live-evidence.json",
    );
    const rawDestination = "https://hooks.internal.example/secret-destination";
    const rawEndpointUrl = "https://provider.internal.example/send";
    const rawMessageBody = "RAW_NOTIFICATION_MESSAGE_BODY_SENTINEL";
    const rawProviderResponse = "RAW_NOTIFICATION_PROVIDER_RESPONSE_SENTINEL";
    const rawSecretRef = "vault://romeo/notifications/provider-secret";
    const rawLogLine = "RAW_NOTIFICATION_LOG_LINE_SENTINEL";
    writeFileSync(
      evidencePath,
      JSON.stringify({
        schemaVersion: "romeo.notification-adapter-live-evidence.v1",
        ...
        rawEvidencePath: evidencePath,
        secretValue: "RAW_NOTIFICATION_SECRET_VALUE_SENTINEL",
      }),
      "utf8",
    );
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        NOTIFICATION_ADAPTER_LIVE_EVIDENCE_PATH: evidencePath,
    ...
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain(rawDestination);
    expect(serialized).not.toContain(rawEndpointUrl);
    expect(serialized).not.toContain(rawMessageBody);
    expect(serialized).not.toContain(rawProviderResponse);
    expect(serialized).not.toContain(rawSecretRef);
    expect(serialized).not.toContain(rawLogLine);
    expect(serialized).not.toContain("RAW_NOTIFICATION_SECRET_VALUE_SENTINEL");
  });

  it("rejects notification adapter live posture evidence that fails GA-aligned required proofs", async () => {
    const directory = mkdtempSync(
      join(tmpdir(), "romeo-notification-live-negative-"),
    );
    const baseEvidence = notificationAdapterLiveEvidence();
    const cases = [
      {
        name: "deployment",
        evidence: notificationAdapterLiveEvidence({
          deployment: "developer-laptop",
        }),
        failureCode: "notification_adapter_live_deployment_invalid",
        warning: "notification_adapter_live_deployment_invalid",
      },
      {
        name: "disabled-delivery",
        evidence: notificationAdapterLiveEvidence({
          delivery: {
            ...baseEvidence.delivery,
            deliveryDriver: "disabled",
          },
        }),
        failureCode: "notification_adapter_live_delivery_invalid",
        warning: "notification_adapter_live_delivery_missing",
      },
      {
        name: "channel-mix",
        evidence: notificationAdapterLiveEvidence({
          channels: {
            ...baseEvidence.channels,
            total: 1,
            mixedChannelTypesVerified: false,
          },
        }),
        failureCode: "notification_adapter_live_channel_mix_invalid",
        warning: "notification_adapter_live_channel_mix_missing",
      },
      {
        name: "policy",
        evidence: notificationAdapterLiveEvidence({
        ...
        failureCode: "notification_adapter_live_policy_invalid",
        warning: "notification_adapter_live_policy_invalid",
      },
    ];
  ...
  it("reports sanitized target quality posture without exposing raw evidence details", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-target-quality-"));
    const evidencePath = join(directory, "target-quality-evidence.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        ...
        target: {
          ...
          origin: "https://romeo.example.com",
          rawUrl: "https://romeo.example.com/secret?token=RAW_TARGET_URL",
        },
        checks: [
          "health_read",
        ...
        analytics: {
          ...
          csv: {
            ...
            sha256:
              "abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789",
            returned: false,
            rawRows: "RAW_ANALYTICS_CSV_SENTINEL",
          },
          redaction: {
        ...
        replay: {
          ...
          averageRecall: 0.75,
          rawQuery: "RAW_REPLAY_QUERY_SENTINEL",
          rawHitId: "RAW_REPLAY_HIT_SENTINEL",
          redaction: {
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        TARGET_QUALITY_EVIDENCE_PATH: evidencePath,
      }),
    });

    const response = await api.request("/api/v1/admin/target-quality/posture");
    const body = await response.json();
    const serialized = JSON.stringify(body);

    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.target-quality-posture.v1");
    expect(body.data.status).toBe("ready");
    ...
    expect(body.data.evals).toMatchObject({
      reportCount: 1,
      passedReportCount: 1,
      gatePassedCount: 1,
    ...
    expect(body.data.replay).toMatchObject({
      checked: true,
      status: "passed",
      kind: "single",
      caseCount: 3,
    ...
    expect(body.data.redaction.rawEvidencePathReturned).toBe(false);
    expect(body.data.redaction.rawTargetUrlReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain("https://romeo.example.com");
    expect(serialized).not.toContain("RAW_TARGET_URL");
    expect(serialized).not.toContain("RAW_ANALYTICS_CSV_SENTINEL");
    expect(serialized).not.toContain("RAW_AGENT_ID_SENTINEL");
    expect(serialized).not.toContain("RAW_WORKSPACE_ID_SENTINEL");
    expect(serialized).not.toContain("RAW_REPLAY_QUERY_SENTINEL");
    expect(serialized).not.toContain("RAW_REPLAY_HIT_SENTINEL");
  });

  it("reports sanitized alert-firing posture without exposing monitoring details", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-alert-firing-"));
    const evidencePath = join(directory, "live-alert-firing.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        ...
        checks: [
          ...
          "alertmanager_alerts_endpoint_read",
          "required_alerts_routed",
        ],
        requiredAlerts: [
          { name: "RAW_PROVIDER_ALERT_SENTINEL", category: "provider" },
          { name: "RAW_QUEUE_ALERT_SENTINEL", category: "queue" },
          { name: "RAW_DEADLETTER_ALERT_SENTINEL", category: "queue" },
          { name: "RAW_BACKUP_ALERT_SENTINEL", category: "backup" },
        ],
        prometheus: {
          status: "passed",
          origin: "https://prometheus.internal.example",
          rawUrl:
            "https://prometheus.internal.example/api/v1/alerts?token=RAW_PROMETHEUS_TOKEN",
          requiredAlertsFiring: [
            {
              name: "RAW_PROVIDER_ALERT_SENTINEL",
              category: "provider",
              severity: "critical",
              activeAt: "2026-07-02T01:55:00.000Z",
              labels: { tenant: "RAW_ALERT_LABEL_SENTINEL" },
              annotations: { summary: "RAW_ALERT_ANNOTATION_SENTINEL" },
            },
            { name: "RAW_QUEUE_ALERT_SENTINEL", category: "queue" },
            { name: "RAW_DEADLETTER_ALERT_SENTINEL", category: "queue" },
            { name: "RAW_BACKUP_ALERT_SENTINEL", category: "backup" },
          ...
          firingAlertCount: 7,
          rawResponse: "RAW_PROMETHEUS_RESPONSE_SENTINEL",
        },
        alertmanager: {
          checked: true,
          status: "passed",
          origin: "https://alertmanager.internal.example",
          requiredAlertsActive: [
            { name: "RAW_PROVIDER_ALERT_SENTINEL", category: "provider" },
            { name: "RAW_QUEUE_ALERT_SENTINEL", category: "queue" },
            { name: "RAW_DEADLETTER_ALERT_SENTINEL", category: "queue" },
            { name: "RAW_BACKUP_ALERT_SENTINEL", category: "backup" },
          ...
          activeAlertCount: 5,
          rawResponse: "RAW_ALERTMANAGER_RESPONSE_SENTINEL",
        },
        redaction: {
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        ALERT_FIRING_EVIDENCE_PATH: evidencePath,
      }),
    });

    const response = await api.request("/api/v1/admin/alert-firing/posture");
    const body = await response.json();
    const serialized = JSON.stringify(body);

    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.alert-firing-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.evidence).toMatchObject({
      configured: true,
      source: "configured_file",
    ...
    expect(body.data.requiredAlerts).toMatchObject({
      total: 4,
    ...
    expect(body.data.prometheus).toEqual({
      checked: true,
      status: "passed",
      originConfigured: true,
      firingAlertCount: 7,
    ...
    expect(body.data.alertmanager).toEqual({
      checked: true,
      status: "passed",
      originConfigured: true,
      activeAlertCount: 5,
    ...
    expect(body.data.redaction.rawEvidencePathReturned).toBe(false);
    expect(body.data.redaction.rawPrometheusUrlReturned).toBe(false);
    expect(body.data.redaction.rawAlertmanagerUrlReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain("prometheus.internal.example");
    expect(serialized).not.toContain("alertmanager.internal.example");
    expect(serialized).not.toContain("RAW_PROVIDER_ALERT_SENTINEL");
    expect(serialized).not.toContain("RAW_ALERT_LABEL_SENTINEL");
    expect(serialized).not.toContain("RAW_ALERT_ANNOTATION_SENTINEL");
    expect(serialized).not.toContain("RAW_PROMETHEUS_RESPONSE_SENTINEL");
    expect(serialized).not.toContain("RAW_ALERTMANAGER_RESPONSE_SENTINEL");
    expect(serialized).not.toContain("RAW_PROMETHEUS_TOKEN");
  });

  it("requires each configured alert to be firing in alert posture evidence", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-alert-firing-names-"));
    const evidencePath = join(directory, "live-alert-firing.json");
    const rawAlertNames = [
      "RAW_REQUIRED_PROVIDER_ALERT",
      "RAW_REQUIRED_QUEUE_ALERT",
      "RAW_REQUIRED_BACKUP_ALERT",
      "RAW_FIRING_PROVIDER_ALERT",
      "RAW_FIRING_QUEUE_ALERT",
      "RAW_FIRING_BACKUP_ALERT",
    ];
    writeFileSync(
      evidencePath,
      JSON.stringify({
        ...
        requiredAlerts: [
          { name: rawAlertNames[0], category: "provider" },
          { name: rawAlertNames[1], category: "queue" },
          { name: rawAlertNames[2], category: "backup" },
        ],
        prometheus: {
          status: "passed",
          origin: "https://prometheus.internal.example",
          requiredAlertsFiring: [
            { name: rawAlertNames[3], category: "provider" },
            { name: rawAlertNames[4], category: "queue" },
            { name: rawAlertNames[5], category: "backup" },
          ],
          firingAlertCount: 3,
        },
        redaction: {
    ...
    expect(body.data.evidence).toMatchObject({
      status: "failed",
      failureCodes: ["required_prometheus_alerts_not_firing"],
    });
    expect(body.data.requiredAlerts).toMatchObject({
      total: 3,
      providerCategoryCount: 1,
      queueCategoryCount: 1,
      backupCategoryCount: 1,
      requiredCategoriesMissing: [],
    });
    expect(body.data.prometheus.requiredFiringCount).toBe(3);
    expect(body.data.prometheus.requiredFiringMissingCount).toBe(3);
    expect(body.data.warnings).toContain(
      "alert_firing_required_alerts_missing",
    );
    for (const value of rawAlertNames) {
      expect(serialized).not.toContain(value);
    }
  });

  it("reports alert-specific missing-check failure codes without leaking alert details", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-alert-firing-checks-"));
    const evidencePath = join(directory, "live-alert-firing.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        schemaVersion: "romeo.live-alert-firing.v1",
        generatedAt: "2026-07-02T02:10:00.000Z",
        status: "passed",
        mode: "live",
        checks: [
          "required_alert_names_defined",
        ...
        requiredAlerts: [
          { name: "RAW_REQUIRED_PROVIDER_ALERT", category: "provider" },
          { name: "RAW_REQUIRED_QUEUE_ALERT", category: "queue" },
          { name: "RAW_REQUIRED_BACKUP_ALERT", category: "backup" },
        ],
        prometheus: {
          status: "passed",
          origin: "https://prometheus.internal.example",
    ...
    expect(response.status).toBe(200);
    expect(body.data.status).toBe("attention_required");
    expect(body.data.evidence.failureCodes).toContain(
      "alert_firing_missing_check:required_prometheus_alerts_firing",
    );
    expect(body.data.warnings).toContain(
      "alert_firing_required_checks_missing",
    );
    expect(serialized).not.toContain("RAW_REQUIRED_PROVIDER_ALERT");
    expect(serialized).not.toContain("prometheus.internal.example");
  });

  it("reports sanitized billing operations posture without exposing target billing details", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-billing-ops-"));
    const evidencePath = join(directory, "billing-operations-evidence.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        schemaVersion: "romeo.billing-operations-evidence.v1",
        ...
        cadence: {
          ...
          observedRunCount: 4,
          missedRunCount: 0,
          rawSchedule: "RAW_BILLING_SCHEDULE_SENTINEL",
        },
        workers: {
          entitlementReconcile: {
            configured: true,
            ...
            failureCount: 0,
            alertConfigured: true,
            rawLog: "RAW_BILLING_WORKER_LOG_SENTINEL",
          },
          lifecycleEnforce: {
            configured: true,
            scheduleConfigured: true,
            ...
            failureCount: 0,
            alertConfigured: true,
            rawApiKey: "RAW_BILLING_API_KEY_SENTINEL",
          },
        },
        apiReadback: {
          entitlementReportHealthy: true,
          ...
          rawCustomerId: "RAW_BILLING_CUSTOMER_SENTINEL",
          rawProviderPayload: "RAW_BILLING_PROVIDER_PAYLOAD_SENTINEL",
        },
        alerting: {
          checked: true,
          status: "passed",
          ...
          firingRequiredCount: 0,
          rawAlertPayload: "RAW_BILLING_ALERT_SENTINEL",
        },
        redaction: {
          rawBillingProviderPayloadsReturned: false,
          rawWorkerLogsReturned: false,
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        BILLING_OPERATIONS_EVIDENCE_PATH: evidencePath,
      }),
    });
    ...
    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.billing-operations-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.evidence).toMatchObject({
      configured: true,
      source: "configured_file",
    ...
    expect(body.data.cadence).toEqual({
      windowMinutes: 120,
      expectedRunCount: 4,
      observedRunCount: 4,
      missedRunCount: 0,
    });
    expect(body.data.workers.entitlementReconcile).toMatchObject({
      scheduleConfigured: true,
      lastRunStatus: "passed",
      successCount: 2,
      alertConfigured: true,
    });
    expect(body.data.apiReadback).toMatchObject({
      entitlementReportHealthy: true,
      lifecycleReportHealthy: true,
      mismatchCount: 0,
      dueTransitionCount: 1,
    });
    expect(body.data.alerting).toMatchObject({
      checked: true,
      status: "passed",
      configuredRuleCount: 2,
    });
    expect(body.data.redaction.rawWorkerLogsReturned).toBe(false);
    expect(body.data.redaction.rawApiKeysReturned).toBe(false);
    expect(body.data.redaction.rawBillingProviderPayloadsReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain("RAW_BILLING_SCHEDULE_SENTINEL");
    expect(serialized).not.toContain("RAW_BILLING_WORKER_LOG_SENTINEL");
    expect(serialized).not.toContain("RAW_BILLING_API_KEY_SENTINEL");
    expect(serialized).not.toContain("RAW_BILLING_CUSTOMER_SENTINEL");
    expect(serialized).not.toContain("RAW_BILLING_PROVIDER_PAYLOAD_SENTINEL");
    expect(serialized).not.toContain("RAW_BILLING_ALERT_SENTINEL");
  });

  it("reports GA-aligned billing operations failure codes", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-billing-ops-bad-"));
    const baseEvidence = billingOperationsEvidence();
    const baseCadence = baseEvidence.cadence as Record<string, unknown>;
    const baseWorkers = baseEvidence.workers as Record<string, unknown>;
    const baseEntitlementWorker = baseWorkers.entitlementReconcile as Record<
      string,
      unknown
    >;
    const cases = [
      {
        evidence: billingOperationsEvidence({ deployment: "unknown" }),
        failureCode: "billing_operations_deployment_invalid",
        warning: "billing_operations_deployment_invalid",
      },
      {
        evidence: billingOperationsEvidence({
          checks: billingOperationsRequiredChecks.filter(
            (check) => check !== "worker_log_redaction",
          ),
        }),
        failureCode: "billing_operations_missing_check:worker_log_redaction",
        warning: "billing_operations_required_checks_missing",
      },
      {
        evidence: billingOperationsEvidence({
          cadence: {
        ...
        failureCode: "billing_operations_cadence_missing",
        warning: "billing_operations_cadence_missing",
      },
      {
        evidence: billingOperationsEvidence({
          workers: {
            ...baseWorkers,
            entitlementReconcile: {
              ...baseEntitlementWorker,
              lastRunStatus: "failed",
              failureCount: 1,
            },
          },
        }),
        failureCode: "billing_operations_entitlement_worker_missing",
        warning: "billing_operations_entitlement_worker_missing",
      },
      {
        evidence: billingOperationsEvidence({
          failures: ["raw_customer_billing_provider_error"],
        }),
        failureCode: "billing_operations_failure_codes_present",
        warning: "billing_operations_failure_codes_present",
      },
    ];

    for (const [index, testCase] of cases.entries()) {
      const evidencePath = join(directory, `billing-operations-${index}.json`);
      writeFileSync(evidencePath, JSON.stringify(testCase.evidence), "utf8");
      const api = createRomeoApi(new InMemoryRomeoRepository(), {
        env: readEnv({
          BILLING_OPERATIONS_EVIDENCE_PATH: evidencePath,
      ...
      expect(serialized).not.toContain("raw_customer_billing_provider_error");
    }
  });

  it("reports sanitized provider outage posture without exposing target provider details", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-provider-outage-"));
    const evidencePath = join(directory, "provider-outage-evidence.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        ...
        drill: {
          ...
          rawProviderPayload: "RAW_PROVIDER_OUTAGE_PAYLOAD_SENTINEL",
        },
        runtime: {
          circuitOpenCount: 1,
          fallbackRoutedCount: 1,
          killSwitchVerifiedCount: 1,
          rawProviderError: "RAW_PROVIDER_OUTAGE_ERROR_SENTINEL",
          rawProviderResponse: "RAW_PROVIDER_OUTAGE_RESPONSE_SENTINEL",
        },
        operationalSummary: {
          checked: true,
          ...
          alertCodeCount: 2,
          rawPrompt: "RAW_PROVIDER_OUTAGE_PROMPT_SENTINEL",
        },
        alerting: {
          checked: true,
          status: "passed",
          ...
          rawAlertPayload: "RAW_PROVIDER_OUTAGE_ALERT_SENTINEL",
        },
        recovery: {
          checked: true,
          recoveredProviderCount: 1,
          recoverySeconds: 45,
          rawApiKey: "RAW_PROVIDER_OUTAGE_API_KEY_SENTINEL",
        },
        redaction: {
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        PROVIDER_OUTAGE_EVIDENCE_PATH: evidencePath,
      }),
    });
    ...
    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.provider-outage-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.evidence).toMatchObject({
      configured: true,
      source: "configured_file",
    ...
    expect(body.data.drill).toEqual({
      providerCount: 2,
      outageInjectedCount: 1,
      timeoutObservedCount: 1,
    });
    expect(body.data.runtime).toEqual({
      circuitOpenCount: 1,
      fallbackRoutedCount: 1,
      killSwitchVerifiedCount: 1,
    });
    expect(body.data.operationalSummary).toMatchObject({
      checked: true,
      degradedProviderCount: 1,
      circuitOpenProviderCount: 1,
      fallbackAvailable: true,
    ...
    expect(body.data.redaction.rawProviderPayloadsReturned).toBe(false);
    expect(body.data.redaction.rawProviderErrorsReturned).toBe(false);
    expect(body.data.redaction.rawProviderResponsesReturned).toBe(false);
    expect(body.data.redaction.rawPromptsReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain("RAW_PROVIDER_OUTAGE_PAYLOAD_SENTINEL");
    expect(serialized).not.toContain("RAW_PROVIDER_OUTAGE_ERROR_SENTINEL");
    expect(serialized).not.toContain("RAW_PROVIDER_OUTAGE_RESPONSE_SENTINEL");
    expect(serialized).not.toContain("RAW_PROVIDER_OUTAGE_PROMPT_SENTINEL");
    expect(serialized).not.toContain("RAW_PROVIDER_OUTAGE_ALERT_SENTINEL");
    expect(serialized).not.toContain("RAW_PROVIDER_OUTAGE_API_KEY_SENTINEL");
  });

  it("reports sanitized tenant purge evidence posture without exposing deletion artifacts", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-tenant-purge-"));
    const evidencePath = join(directory, "tenant-purge-evidence.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        ...
        purge: {
          ...
          rawObjectKey: "files/org_acme/private/key.txt",
        },
        storage: {
          ...
          secretStoreCount: 2,
          rawVectorId: "RAW_TENANT_PURGE_VECTOR_SENTINEL",
        },
        retention: {
          backupRetentionDays: 90,
          operationalLogRetentionDays: 30,
          supportBundleRetentionDays: 14,
          backupLocation: "s3://tenant-backups/org_acme",
        },
        redaction: {
        ...
        rawEvidencePath: evidencePath,
        rawOperationalLogBody: "RAW_TENANT_PURGE_LOG_SENTINEL",
        rawSupportBundleBody: "RAW_TENANT_PURGE_SUPPORT_SENTINEL",
        secretValue: "RAW_TENANT_PURGE_SECRET_SENTINEL",
      }),
      "utf8",
    );
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        TENANT_PURGE_EVIDENCE_PATH: evidencePath,
      }),
    });
    ...
    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.tenant-purge-evidence-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.evidence).toMatchObject({
      configured: true,
      source: "configured_file",
    ...
    expect(body.data.purge).toMatchObject({
      tenantCount: 1,
      databasePurgedTenantCount: 1,
      objectStorePurgedTenantCount: 1,
      externalSecretReviewedTenantCount: 1,
    });
    expect(body.data.storage).toMatchObject({
      postgresRecordCount: 42,
      objectStoreObjectCount: 7,
      secretStoreCount: 2,
    });
    ...
    expect(body.data.redaction.objectStoreKeysReturned).toBe(false);
    expect(body.data.redaction.vectorValuesReturned).toBe(false);
    expect(body.data.redaction.secretValuesReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain("files/org_acme/private/key.txt");
    expect(serialized).not.toContain("s3://tenant-backups/org_acme");
    expect(serialized).not.toContain("RAW_TENANT_PURGE_VECTOR_SENTINEL");
    expect(serialized).not.toContain("RAW_TENANT_PURGE_LOG_SENTINEL");
    expect(serialized).not.toContain("RAW_TENANT_PURGE_SUPPORT_SENTINEL");
    expect(serialized).not.toContain("RAW_TENANT_PURGE_SECRET_SENTINEL");
  });

  it("reports GA-aligned tenant purge evidence failure codes", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-tenant-purge-bad-"));
    const baseEvidence = {
      schemaVersion: "romeo.tenant-purge-evidence.v1",
      generatedAt: "2026-07-06T20:45:00.000Z",
      status: "passed",
      mode: "live",
      deployment: "kubernetes",
      checks: [
    ...
    const cases = [
      {
        ...
        failureCode: "tenant_purge_deployment_invalid",
        warning: "tenant_purge_deployment_invalid",
      },
      {
        evidence: {
          ...baseEvidence,
          checks: baseEvidence.checks.filter(
            (check) => check !== "external_vector_store_reviewed",
          ),
        },
        failureCode:
          "tenant_purge_missing_check:external_vector_store_reviewed",
        warning: "tenant_purge_required_checks_missing",
      },
      {
        evidence: {
          ...baseEvidence,
          storage: {
            ...baseEvidence.storage,
            objectStoreObjectCount: 0,
          },
        },
        failureCode: "tenant_purge_objectStoreObjectCount_missing",
        warning: "tenant_purge_storage_review_missing",
      },
      {
        evidence: {
          ...baseEvidence,
        ...
        failureCode: "tenant_purge_backupRetentionDays_missing",
        warning: "tenant_purge_retention_days_missing",
      },
      {
        evidence: {
          ...baseEvidence,
          failures: ["raw_customer_tenant_purge_failure"],
        },
        failureCode: "tenant_purge_failure_codes_present",
        warning: "tenant_purge_failure_codes_present",
      },
    ];

    for (const [index, testCase] of cases.entries()) {
      const evidencePath = join(directory, `tenant-purge-${index}.json`);
      writeFileSync(evidencePath, JSON.stringify(testCase.evidence), "utf8");
      const api = createRomeoApi(new InMemoryRomeoRepository(), {
        env: readEnv({
          TENANT_PURGE_EVIDENCE_PATH: evidencePath,
      ...
      expect(body.data.evidence.failureCodes).toContain(testCase.failureCode);
      expect(body.data.warnings).toContain(testCase.warning);
      expect(serialized).not.toContain("raw_customer_tenant_purge_failure");
    }
  });

  it("reports sanitized identity live posture without exposing directory or IdP artifacts", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-identity-live-"));
    const evidencePath = join(directory, "identity-live-evidence.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        schemaVersion: "romeo.identity-live-evidence.v1",
        generatedAt: "2026-07-06T21:15:00.000Z",
        status: "passed",
        mode: "live",
        deployment: "kubernetes",
        checks: [
        ...
        identityProviders: {
          ...
          oidcProviderCount: 1,
          oauth2ProviderCount: 1,
          ldapProviderCount: 1,
          samlProviderCount: 0,
          localFallbackVerified: true,
          mfaFallbackVerified: true,
          rawProviderEndpoint:
            "https://idp.example.mil/.well-known/openid-configuration",
          rawIdpResponse: "RAW_IDENTITY_IDP_RESPONSE_SENTINEL",
        },
        secretBackends: {
          managedSecretBackendCount: 2,
          vaultSecretWriteCount: 1,
          externalSecretReferenceCount: 1,
          secretResolutionCheckCount: 2,
          rawSecretRef: "vault://romeo/auth/client-secret",
        },
        directory: {
          directoryProviderCount: 1,
          directoryLookupCount: 4,
          mappedGroupCount: 2,
          workspaceMappingCount: 2,
          directorySyncPreviewChangeCount: 3,
          directorySyncAppliedChangeCount: 3,
          policyViolationCount: 0,
          rawDirectoryEntry: "cn=Jane Doe,ou=Users,dc=example,dc=mil",
          rawGroupName: "Privileged Identity Operators",
          rawEmail: "jane.doe@example.mil",
        },
        lifecycle: {
          ...
          rawSamlAssertion: "RAW_IDENTITY_SAML_ASSERTION_SENTINEL",
        },
        accessReview: {
          checked: true,
          reportUserCount: 10,
        ...
        rawEvidencePath: evidencePath,
        tokenValue: "RAW_IDENTITY_TOKEN_SENTINEL",
        secretValue: "RAW_IDENTITY_SECRET_SENTINEL",
      }),
      "utf8",
    );
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        IDENTITY_LIVE_EVIDENCE_PATH: evidencePath,
      }),
    });

    const response = await api.request("/api/v1/admin/identity/live-posture");
    const body = await response.json();
    const serialized = JSON.stringify(body);

    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.identity-live-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.evidence).toMatchObject({
      configured: true,
      source: "configured_file",
    ...
    expect(body.data.identityProviders).toMatchObject({
      configuredProviderCount: 3,
      liveLoginProviderCount: 2,
      localFallbackVerified: true,
      mfaFallbackVerified: true,
    });
    expect(body.data.secretBackends).toMatchObject({
      managedSecretBackendCount: 2,
      vaultSecretWriteCount: 1,
      secretResolutionCheckCount: 2,
    });
    expect(body.data.directory).toMatchObject({
      directoryProviderCount: 1,
      directorySyncAppliedChangeCount: 3,
      policyViolationCount: 0,
    });
    expect(body.data.accessReview).toMatchObject({
      checked: true,
      reportUserCount: 10,
      exportedCsv: true,
    });
    expect(body.data.redaction.rawSecretRefsReturned).toBe(false);
    expect(body.data.redaction.rawSamlAssertionsReturned).toBe(false);
    expect(body.data.redaction.rawEmailAddressesReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain("https://idp.example.mil");
    expect(serialized).not.toContain("RAW_IDENTITY_IDP_RESPONSE_SENTINEL");
    expect(serialized).not.toContain("vault://romeo/auth/client-secret");
    expect(serialized).not.toContain("cn=Jane Doe");
    expect(serialized).not.toContain("Privileged Identity Operators");
    expect(serialized).not.toContain("jane.doe@example.mil");
    expect(serialized).not.toContain("RAW_IDENTITY_SAML_ASSERTION_SENTINEL");
    expect(serialized).not.toContain("RAW_IDENTITY_TOKEN_SENTINEL");
    expect(serialized).not.toContain("RAW_IDENTITY_SECRET_SENTINEL");
  });

  it("rejects identity live posture evidence missing GA-aligned deployment or fallback proof", async () => {
    const directory = mkdtempSync(
      join(tmpdir(), "romeo-identity-live-negative-"),
    );
    const baseEvidence = () => ({
      schemaVersion: "romeo.identity-live-evidence.v1",
      generatedAt: "2026-07-06T21:15:00.000Z",
    ...
    const cases = [
      {
        name: "deployment",
        evidence: { ...baseEvidence(), deployment: "developer-laptop" },
        failureCode: "identity_live_deployment_invalid",
        warning: "identity_live_deployment_invalid",
      },
      {
        name: "mfa-fallback",
        evidence: {
          ...baseEvidence(),
          identityProviders: {
            ...baseEvidence().identityProviders,
            mfaFallbackVerified: false,
          },
        },
        failureCode: "identity_live_provider_login_missing",
        warning: "identity_live_login_missing",
      },
    ];
  ...
  it("reports sanitized analytics authz posture without exposing target artifacts", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-analytics-authz-"));
    const evidencePath = join(directory, "analytics-authz-live-evidence.json");
    writeFileSync(
      evidencePath,
      JSON.stringify(
        analyticsAuthzLiveEvidence({
          deployment: "target",
          subjects: {
            adminSubjectCount: 2,
            orgAdminSubjectCount: 1,
            nonAdminSubjectCount: 2,
            serviceAccountSubjectCount: 1,
            crossOrgSubjectCount: 1,
            rawUserEmail: "analyst@example.mil",
          },
          authorization: {
            adminSummaryAllowedCount: 2,
            adminCsvAllowedCount: 2,
            nonAdminSummaryDeniedCount: 1,
            nonAdminCsvDeniedCount: 1,
            missingUsageScopeDeniedCount: 1,
            evalGrantDeniedCount: 1,
            crossOrgDeniedCount: 1,
            crossWorkspaceScopedCount: 1,
            rawWorkspaceName: "Restricted Analytics Workspace",
            rawOrgName: "Restricted Analytics Org",
          },
          analytics: {
            summaryReadCount: 2,
            csvExportReadCount: 2,
            evalEvidenceReadCount: 1,
            csvSha256Count: 2,
            usageMetricCount: 12,
            evalSuiteCount: 3,
            jobSummaryCount: 1,
            providerSummaryCount: 1,
            rawCsv: "RAW_ANALYTICS_CSV_SENTINEL",
            rawEvalInput: "RAW_ANALYTICS_EVAL_INPUT_SENTINEL",
            rawEvalOutput: "RAW_ANALYTICS_EVAL_OUTPUT_SENTINEL",
            rawUsageMetadata: "RAW_ANALYTICS_USAGE_SENTINEL",
            rawJobPayload: "RAW_ANALYTICS_JOB_SENTINEL",
            rawProviderConfig: "RAW_ANALYTICS_PROVIDER_SENTINEL",
          },
          rawEvidencePath: evidencePath,
          apiKey: "RAW_ANALYTICS_API_KEY_SENTINEL",
          tokenValue: "RAW_ANALYTICS_TOKEN_SENTINEL",
          secretRef: "vault://romeo/analytics/export-secret",
          secretValue: "RAW_ANALYTICS_SECRET_SENTINEL",
        }),
      ),
      "utf8",
    );
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        ANALYTICS_AUTHZ_EVIDENCE_PATH: evidencePath,
      }),
    });

    const response = await api.request("/api/v1/admin/analytics/authz-posture");
    const body = await response.json();
    const serialized = JSON.stringify(body);

    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.analytics-authz-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.evidence).toMatchObject({
      configured: true,
      source: "configured_file",
    ...
    expect(body.data.subjects).toMatchObject({
      adminSubjectCount: 2,
      nonAdminSubjectCount: 2,
      crossOrgSubjectCount: 1,
    });
    expect(body.data.authorization).toMatchObject({
      adminSummaryAllowedCount: 2,
      nonAdminCsvDeniedCount: 1,
      missingUsageScopeDeniedCount: 1,
      evalGrantDeniedCount: 1,
    ...
    expect(body.data.redaction.rawAnalyticsCsvRowsReturned).toBe(false);
    expect(body.data.redaction.rawUsageMetadataReturned).toBe(false);
    expect(body.data.redaction.apiKeysReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain("RAW_ANALYTICS_CSV_SENTINEL");
    expect(serialized).not.toContain("RAW_ANALYTICS_EVAL_INPUT_SENTINEL");
    expect(serialized).not.toContain("RAW_ANALYTICS_EVAL_OUTPUT_SENTINEL");
    expect(serialized).not.toContain("RAW_ANALYTICS_USAGE_SENTINEL");
    expect(serialized).not.toContain("RAW_ANALYTICS_JOB_SENTINEL");
    expect(serialized).not.toContain("RAW_ANALYTICS_PROVIDER_SENTINEL");
    expect(serialized).not.toContain("analyst@example.mil");
    expect(serialized).not.toContain("Restricted Analytics Workspace");
    expect(serialized).not.toContain("Restricted Analytics Org");
    expect(serialized).not.toContain("vault://romeo/analytics/export-secret");
    expect(serialized).not.toContain("RAW_ANALYTICS_API_KEY_SENTINEL");
    expect(serialized).not.toContain("RAW_ANALYTICS_TOKEN_SENTINEL");
    expect(serialized).not.toContain("RAW_ANALYTICS_SECRET_SENTINEL");
  });

  it("rejects analytics authz posture evidence missing GA-aligned deployment", async () => {
    const directory = mkdtempSync(
      join(tmpdir(), "romeo-analytics-authz-negative-"),
    );
    const evidencePath = join(directory, "analytics-authz-live-evidence.json");
    writeFileSync(
      evidencePath,
      JSON.stringify(analyticsAuthzLiveEvidence({ deployment: "compose" })),
      "utf8",
    );
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
  ...
  it("reports sanitized migration drill posture without exposing migration details", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-migration-drill-"));
    const evidencePath = join(directory, "migration-drill-evidence.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        ...
        drill: {
          ...
          cutoverBlocked: true,
          rawDatabaseUrl: "postgres://romeo:RAW_MIGRATION_DB_PASSWORD@db/romeo",
        },
        job: {
          ...
          rawMigrationSql: "ALTER TABLE RAW_MIGRATION_SQL_SENTINEL",
        },
        validation: {
          ...
          rawErrorStack: "RAW_MIGRATION_STACK_SENTINEL",
        },
        runbook: {
          reviewed: true,
          recoveryDocumented: true,
          reviewerCount: 2,
          rawMigrationLog: "RAW_MIGRATION_LOG_SENTINEL",
        },
        redaction: {
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        MIGRATION_DRILL_EVIDENCE_PATH: evidencePath,
      }),
    });
    ...
    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.migration-drill-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.evidence).toMatchObject({
      configured: true,
      source: "configured_file",
    ...
    expect(body.data.runbook).toEqual({
      reviewed: true,
      recoveryDocumented: true,
      reviewerCount: 2,
    });
    expect(body.data.redaction.databaseUrlsReturned).toBe(false);
    expect(body.data.redaction.migrationSqlReturned).toBe(false);
    expect(body.data.redaction.migrationLogsReturned).toBe(false);
    expect(body.data.redaction.rawErrorStacksReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain("RAW_MIGRATION_DB_PASSWORD");
    expect(serialized).not.toContain("RAW_MIGRATION_SQL_SENTINEL");
    expect(serialized).not.toContain("RAW_MIGRATION_STACK_SENTINEL");
    expect(serialized).not.toContain("RAW_MIGRATION_LOG_SENTINEL");
  });

  it("reports sanitized network partition posture without exposing network details", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-network-partition-"));
    const evidencePath = join(directory, "network-partition-evidence.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        schemaVersion: "romeo.network-partition-evidence.v1",
        generatedAt: "2026-07-06T18:50:00.000Z",
        status: "passed",
        mode: "live",
        deployment: "kubernetes",
        checks: [
        ...
        drill: {
          ...
          partitionDurationSeconds: 90,
          rawNetworkEndpoint: "https://RAW_NETWORK_ENDPOINT_SENTINEL",
        },
        runtime: {
          apiDegraded: true,
          failClosedCount: 3,
          ...
          rawPacketCapture: "RAW_PACKET_CAPTURE_SENTINEL",
        },
        recovery: {
          checked: true,
          ...
          rawPodIp: "10.42.0.99-RAW_POD_IP_SENTINEL",
        },
        alerting: {
          checked: true,
          status: "passed",
          ...
          firingRequiredCount: 1,
          rawLogLine: "RAW_NETWORK_LOG_SENTINEL",
        },
        networkContext: {
          cniConfirmed: true,
          ...
          egressPolicyCount: 2,
          rawSecret: "RAW_NETWORK_SECRET_SENTINEL",
        },
        redaction: {
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        NETWORK_PARTITION_EVIDENCE_PATH: evidencePath,
      }),
    });
    ...
    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.network-partition-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.evidence).toMatchObject({
      configured: true,
      source: "configured_file",
    ...
    expect(body.data.networkContext).toEqual({
      cniConfirmed: true,
      networkPolicyApplied: true,
      namespaceScoped: true,
      egressPolicyCount: 2,
    });
    expect(body.data.redaction.rawNetworkEndpointsReturned).toBe(false);
    expect(body.data.redaction.rawPodIpsReturned).toBe(false);
    expect(body.data.redaction.rawPacketCapturesReturned).toBe(false);
    expect(body.data.redaction.rawLogLinesReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain("RAW_NETWORK_ENDPOINT_SENTINEL");
    expect(serialized).not.toContain("RAW_PACKET_CAPTURE_SENTINEL");
    expect(serialized).not.toContain("RAW_POD_IP_SENTINEL");
    expect(serialized).not.toContain("RAW_NETWORK_LOG_SENTINEL");
    expect(serialized).not.toContain("RAW_NETWORK_SECRET_SENTINEL");
  });

  it("reports sanitized secret rotation drill posture without exposing secrets", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-secret-rotation-"));
    const evidencePath = join(directory, "secret-rotation-drill-evidence.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        schemaVersion: "romeo.secret-rotation-drill-evidence.v1",
        generatedAt: "2026-07-06T19:20:00.000Z",
        status: "passed",
        mode: "live",
        deployment: "kubernetes",
        checks: [
        ...
        stagedCutover: {
          ...
          rawSessionSecret: "RAW_SESSION_SECRET_SENTINEL",
          rawWebhookSigningSecret: "RAW_WEBHOOK_SIGNING_SECRET_SENTINEL",
        },
        rewrap: {
          ...
          failureCount: 0,
          rawKeyMaterial: "RAW_KEY_MATERIAL_SENTINEL",
          rawSecretRef: "romeo-secret://RAW_SECRET_REF_SENTINEL",
        },
        acceptance: {
          oldSecretRetiredOrRejectedCount: 2,
          newSecretAcceptedCount: 2,
          rawToken: "RAW_TOKEN_SENTINEL",
        },
        dependencies: {
          ...
          rawApiKey: "RAW_API_KEY_SENTINEL",
        },
        readiness: {
          checked: true,
          ...
          rawSecretValue: "RAW_SECRET_VALUE_SENTINEL",
        },
        alerting: {
          checked: true,
          status: "passed",
          ...
          firingRequiredCount: 1,
          rawLogLine: "RAW_SECRET_ROTATION_LOG_SENTINEL",
        },
        redaction: {
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        SECRET_ROTATION_DRILL_EVIDENCE_PATH: evidencePath,
      }),
    });
    ...
    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.secret-rotation-drill-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.evidence).toMatchObject({
      configured: true,
      source: "configured_file",
    ...
    expect(body.data.stagedCutover).toEqual({
      sessionSecretStaged: true,
      webhookSigningKeyCutover: true,
      apiOrServiceKeyContinuityVerified: true,
    });
    expect(body.data.rewrap).toEqual({
      localMfaPreviewPassed: true,
      localMfaRewrappedCount: 2,
      managedSecretsPreviewPassed: true,
      managedSecretsRewrappedCount: 3,
    ...
    expect(body.data.acceptance).toEqual({
      oldSecretRetiredOrRejectedCount: 2,
      newSecretAcceptedCount: 2,
    });
    ...
    expect(body.data.redaction.rawSecretValuesReturned).toBe(false);
    expect(body.data.redaction.rawSecretRefsReturned).toBe(false);
    expect(body.data.redaction.keyMaterialReturned).toBe(false);
    expect(body.data.redaction.webhookSigningSecretsReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(evidencePath);
    expect(serialized).not.toContain(directory);
    expect(serialized).not.toContain("RAW_SESSION_SECRET_SENTINEL");
    expect(serialized).not.toContain("RAW_WEBHOOK_SIGNING_SECRET_SENTINEL");
    expect(serialized).not.toContain("RAW_KEY_MATERIAL_SENTINEL");
    expect(serialized).not.toContain("RAW_SECRET_REF_SENTINEL");
    expect(serialized).not.toContain("RAW_TOKEN_SENTINEL");
    expect(serialized).not.toContain("RAW_API_KEY_SENTINEL");
    expect(serialized).not.toContain("RAW_SECRET_VALUE_SENTINEL");
    expect(serialized).not.toContain("RAW_SECRET_ROTATION_LOG_SENTINEL");
  });

  it("reports sanitized Kubernetes evidence posture without exposing mounted evidence details", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-kubernetes-posture-"));
    const evidencePaths = {
      live: join(directory, "kubernetes-live-smoke.json"),
      workers: join(directory, "kubernetes-workers-smoke.json"),
      networkpolicy: join(directory, "kubernetes-networkpolicy-smoke.json"),
      cloudnativepgDr: join(directory, "kubernetes-cloudnativepg-dr.json"),
      externalPostgresDr: join(
        directory,
        "kubernetes-external-postgres-dr.json",
      ),
      tieredRag: join(directory, "kubernetes-tiered-rag-smoke.json"),
      loadSoak: join(directory, "kubernetes-load-soak.json"),
    };
    const rawSentinels = {
      databaseUrl: "postgres://romeo:RAW_DB_PASSWORD@db.internal/romeo",
      image: "registry.internal.example/romeo/app:RAW_IMAGE_TAG",
      namespace: "romeo-RAW_NAMESPACE",
      podLog: "RAW_KUBERNETES_POD_LOG",
      secret: "RAW_KUBERNETES_SECRET",
    };
    const writeEvidence = (path: string, data: Record<string, unknown>) =>
      writeFileSync(
        path,
        JSON.stringify({
          ...
          target: {
            deployment: "kubernetes",
            namespace: rawSentinels.namespace,
            releaseName: "romeo",
            serviceName: "romeo",
            deploymentName: "romeo",
            appName: "romeo",
            image: rawSentinels.image,
          },
          rawPodLog: rawSentinels.podLog,
          databaseUrl: rawSentinels.databaseUrl,
          secretValue: rawSentinels.secret,
          ...data,
        }),
        "utf8",
      );
    writeEvidence(evidencePaths.live, {
      ...
      productWorkflow: {
        chatId: "chat_kubernetes",
        sourceId: "source_kubernetes",
        runId: "run_kubernetes",
        webhookDeliveryId: "delivery_kubernetes",
      },
      logRedaction: {
        status: "passed",
    ...
    writeEvidence(evidencePaths.workers, {
      ...
      controlledWorkflowResume: {
        workflowId: "workflow_kubernetes",
        workflowRunId: "workflow_run_kubernetes",
        linkedRunId: "run_worker_linked",
      },
      crashRecovery: {
    ...
    writeEvidence(evidencePaths.networkpolicy, {
    ...
    for (const [databaseMode, path] of [
      ["cloudnativepg", evidencePaths.cloudnativepgDr],
      ["external-postgres", evidencePaths.externalPostgresDr],
    ] as const) {
      writeEvidence(path, {
        ...
        restore: {
          namespace: `${rawSentinels.namespace}-restore`,
          databaseConnection: {
            source:
              databaseMode === "cloudnativepg"
                ? "operator_secret"
    ...
    writeEvidence(evidencePaths.tieredRag, {
    ...
    writeEvidence(evidencePaths.loadSoak, {
      schemaVersion: "romeo.kubernetes-load-soak.v1",
      tier: "small",
      iterations: 2,
      loadRuns: 2,
      soak: { requestedSeconds: 60, observedSeconds: 61, passed: true },
      checks: [
        "cluster_reachable",
        "namespace_readable",
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        KUBERNETES_LIVE_SMOKE_EVIDENCE_PATH: evidencePaths.live,
        KUBERNETES_WORKERS_EVIDENCE_PATH: evidencePaths.workers,
        KUBERNETES_NETWORKPOLICY_EVIDENCE_PATH: evidencePaths.networkpolicy,
        KUBERNETES_CLOUDNATIVEPG_DR_EVIDENCE_PATH:
          evidencePaths.cloudnativepgDr,
        KUBERNETES_EXTERNAL_POSTGRES_DR_EVIDENCE_PATH:
          evidencePaths.externalPostgresDr,
        KUBERNETES_TIERED_RAG_EVIDENCE_PATH: evidencePaths.tieredRag,
        KUBERNETES_LOAD_SOAK_EVIDENCE_PATH: evidencePaths.loadSoak,
      }),
    });

    const response = await api.request("/api/v1/admin/kubernetes/posture");
    const body = await response.json();
    const serialized = JSON.stringify(body);

    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.kubernetes-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.summary).toMatchObject({
      requiredTotal: 7,
      requiredSatisfied: 7,
    ...
    expect(body.data.evidence).toHaveLength(9);
    expect(
      body.data.evidence.find(
        (item: { kind: string }) => item.kind === "cloudnativepg_dr",
      ),
    ).toMatchObject({
      status: "satisfied",
      databaseMode: "cloudnativepg",
      checks: { missingRequired: [] },
    });
    expect(
      body.data.evidence.find(
        (item: { kind: string }) => item.kind === "load_soak",
      ),
    ).toMatchObject({
      metrics: {
        iterationCount: 2,
        loadRunCount: 2,
        soakObservedSeconds: 61,
        soakRequestedSeconds: 60,
      },
    });
    expect(body.data.warnings).toEqual([]);
    expect(body.data.redaction.rawEvidencePathsReturned).toBe(false);
    expect(body.data.redaction.podLogsReturned).toBe(false);
    expect(body.data.redaction.rawImageRefsReturned).toBe(false);
    for (const value of Object.values(evidencePaths)) {
      expect(serialized).not.toContain(value);
    }
    expect(serialized).not.toContain(directory);
    for (const value of Object.values(rawSentinels)) {
      expect(serialized).not.toContain(value);
    }
  });

  it("rejects Kubernetes posture evidence that is not from a Kubernetes target", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-kubernetes-target-"));
    const evidencePath = join(directory, "kubernetes-live-smoke.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        schemaVersion: "romeo.kubernetes-live-smoke.v1",
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        KUBERNETES_LIVE_SMOKE_EVIDENCE_PATH: evidencePath,
      }),
    });

    const response = await api.request("/api/v1/admin/kubernetes/posture");
    const body = await response.json();
    const liveSmoke = body.data.evidence.find(
      (item: { kind: string }) => item.kind === "live_smoke",
    );

    expect(response.status).toBe(200);
    expect(body.data.status).toBe("attention_required");
    expect(liveSmoke).toMatchObject({
      configured: true,
      status: "failed",
      target: { deployment: "unknown" },
    });
    expect(liveSmoke.failureCodes).toContain("target_not_kubernetes");
    expect(body.data.warnings).toContain("kubernetes_required_evidence_failed");
  });

  it("rejects Kubernetes NetworkPolicy posture evidence with stale contract metadata", async () => {
    const directory = mkdtempSync(
      join(tmpdir(), "romeo-kubernetes-networkpolicy-posture-"),
    );
    const evidencePath = join(directory, "kubernetes-networkpolicy-smoke.json");
    writeFileSync(
      evidencePath,
      JSON.stringify({
        schemaVersion: "romeo.kubernetes-networkpolicy-smoke.v1",
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        KUBERNETES_NETWORKPOLICY_EVIDENCE_PATH: evidencePath,
      }),
    });

    const response = await api.request("/api/v1/admin/kubernetes/posture");
    const body = await response.json();
    const networkPolicy = body.data.evidence.find(
      (item: { kind: string }) => item.kind === "networkpolicy",
    );
    ...
    expect(networkPolicy.failureCodes).toContain(
      "networkpolicy_enforcement_contract_missing",
    );
    expect(networkPolicy.failureCodes).toContain(
      "networkpolicy_unexpected_selected_component",
    );
    expect(body.data.warnings).toContain("kubernetes_required_evidence_failed");
  });

  it("reports sanitized CI governance posture from mounted files", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-ci-governance-"));
    const evidencePaths = {
      branchPlan: join(directory, "branch-protection-plan.json"),
      branchVerification: join(
        directory,
        "branch-protection-verification.json",
      ),
      hostedRun: join(directory, "hosted-ci-run-verification.json"),
    };
    const rawSentinels = {
      apiResponse: "RAW_GITHUB_API_RESPONSE_BODY",
      branchName: "RAW_BRANCH_NAME",
      jobLog: "RAW_JOB_LOG_BODY",
      repositorySlug: "RAW_REPOSITORY_SLUG",
      runUrl: "https://github.com/RAW_RUN_URL",
      statusCheckName: "RAW_STATUS_CHECK_NAME",
      token: "RAW_GITHUB_TOKEN",
      workflowBody: "RAW_WORKFLOW_BODY",
    };
    const policy = {
      targetBranch: rawSentinels.branchName,
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        workflow: {
          ...
          name: "Romeo CI",
          jobCount: 5,
          rawBody: rawSentinels.workflowBody,
        },
        policy,
        requiredStatusChecks: [
          {
            workflow: "Romeo CI",
            jobId: "quality",
            name: rawSentinels.statusCheckName,
            required: true,
          },
          {
            workflow: "Romeo CI",
        ...
        checks: [
          { name: "workflow runs pnpm check", status: "pass" },
          { name: "workflow runs pnpm test", status: "pass" },
          {
            name: `required CI job includes ${rawSentinels.statusCheckName}`,
            status: "pass",
          },
        ],
        blockers: [],
        ...
        rawTokenValue: rawSentinels.token,
      }),
      "utf8",
    );
    writeFileSync(
      evidencePaths.hostedRun,
      JSON.stringify({
        ...
        target: {
          branch: rawSentinels.branchName,
          repositorySlug: rawSentinels.repositorySlug,
          repositorySlugReturned: false,
          runUrl: rawSentinels.runUrl,
        },
        plan: {
          file: evidencePaths.branchPlan,
          status: "passed",
        ...
        checks: [
          ...
          { name: "workflow run conclusion successful", status: "pass" },
          {
            ...
            status: "pass",
            missingCount: 0,
            missingNames: [rawSentinels.statusCheckName],
          },
          {
            name: "all required CI jobs successful",
            status: "pass",
            failedCount: 0,
            failedNames: [rawSentinels.statusCheckName],
          },
          {
            name: "hosted CI job inventory read",
            status: "pass",
            jobCount: 17,
            rawJobLog: rawSentinels.jobLog,
          },
        ],
        blockers: [],
        redaction: {
        ...
        rawApiResponse: rawSentinels.apiResponse,
        rawTokenValue: rawSentinels.token,
      }),
      "utf8",
    );
    writeFileSync(
      evidencePaths.branchVerification,
      JSON.stringify({
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        CI_BRANCH_PROTECTION_PLAN_PATH: evidencePaths.branchPlan,
        CI_HOSTED_RUN_VERIFICATION_PATH: evidencePaths.hostedRun,
        CI_BRANCH_PROTECTION_VERIFICATION_PATH:
          evidencePaths.branchVerification,
      }),
    });

    const response = await api.request("/api/v1/admin/ci-governance/posture");
    const body = await response.json();
    const serialized = JSON.stringify(body);

    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.ci-governance-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.summary.planReady).toBe(true);
    expect(body.data.summary.hostedRunVerified).toBe(true);
    expect(body.data.summary.branchProtectionVerified).toBe(true);
    expect(body.data.summary.requiredStatusCheckCount).toBe(2);
    expect(body.data.summary.requiredWorkflowCommandCount).toBe(2);
    expect(body.data.hostedRun.jobs.observedJobCount).toBe(17);
    expect(body.data.branchProtection.controls.passedCount).toBe(11);
    expect(body.data.redaction.repositorySlugsReturned).toBe(false);
    expect(body.data.redaction.runUrlsReturned).toBe(false);
    expect(body.data.redaction.workflowBodiesReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(directory);
    for (const value of Object.values(rawSentinels)) {
      expect(serialized).not.toContain(value);
  ...
  it("reports sanitized release security posture from mounted files", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-release-security-"));
    const evidencePaths = {
      airgap: join(directory, "airgap-bundle-verification.json"),
      approval: join(directory, "release-approval.json"),
      provenance: join(directory, "release-provenance.json"),
      publishPlan: join(directory, "publish-plan.json"),
    };
    const rawSentinels = {
      approvalRef: "RAW_RELEASE_APPROVAL_REF",
      approverId: "RAW_RELEASE_APPROVER_ID",
      bundlePath: "/RAW_AIRGAP_BUNDLE_PATH",
      ciRunUrl: "https://ci.example.com/RAW_RELEASE_CI_RUN",
      command: "RAW_RELEASE_COMMAND",
      registryUrl: "https://registry.example.com/RAW_REGISTRY_URL",
      secret: "RAW_RELEASE_SECURITY_SECRET",
      signatureBody: "RAW_SIGNATURE_BODY",
      sourceRef: "refs/heads/RAW_SOURCE_REF",
      sourceRepo: "RAW_SOURCE_REPOSITORY",
      token: "RAW_RELEASE_SECURITY_TOKEN",
    };

    writeFileSync(
      evidencePaths.provenance,
      JSON.stringify({
        schemaVersion: "romeo.release-provenance.v1",
        ...
        source: {
          ...
          sourceRepoHash: "b".repeat(64),
          sourceRefConfigured: true,
          sourceRefHash: "c".repeat(64),
          builderIdConfigured: true,
          builderIdHash: "d".repeat(64),
          ciRunUrlConfigured: true,
          ciRunUrlHash: "e".repeat(64),
          rawSourceRepo: rawSentinels.sourceRepo,
          rawSourceRef: rawSentinels.sourceRef,
          rawCiRunUrl: rawSentinels.ciRunUrl,
        },
        supplyChain: {
          sbomAttached: true,
        ...
        signature: {
          signed: true,
          signatureBody: rawSentinels.signatureBody,
        },
        checks: [
          { name: "release manifest name is present", status: "pass" },
          { name: "provenance redaction self-check passed", status: "pass" },
        ],
        blockers: [],
        redaction: {
          tokenValuesReturned: false,
        ...
        token: rawSentinels.token,
        secret: rawSentinels.secret,
      }),
      "utf8",
    );
    writeFileSync(
      evidencePaths.approval,
      JSON.stringify({
        ...
        approval: {
          ...
          approverCount: 2,
          minApprovers: 2,
          approverHashes: ["1".repeat(64), "2".repeat(64)],
          approvedAt: "2026-07-03T05:04:00.000Z",
          rawApprovalRef: rawSentinels.approvalRef,
          rawApproverIds: [rawSentinels.approverId],
        },
        checks: [
          {
            name: "release approval has enough unique approvers",
            status: "pass",
          },
        ],
        blockers: [],
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        release: {
          ...
          version: "1.2.3",
          artifacts: [
            { name: "@romeo/api-client", version: "1.2.3" },
            { name: "@romeo/core", version: "1.2.3" },
          ],
          securityEvidence: { sha256: "3".repeat(64) },
          provenance: { sha256: "4".repeat(64) },
          approval: { sha256: "5".repeat(64) },
          releaseNotes: { sha256: "6".repeat(64) },
        },
        policy: {
          registryUrl: rawSentinels.registryUrl,
          npmProvenance: true,
        ...
        steps: [
          {
            ...
            command: ["pnpm", "publish", rawSentinels.registryUrl],
          },
          { kind: "registry_publish", command: [rawSentinels.command] },
          { kind: "git_tag", command: [rawSentinels.command] },
          { kind: "git_push", command: [rawSentinels.command] },
          { kind: "release_notes", command: [rawSentinels.command] },
        ],
        blockers: [],
      }),
      "utf8",
      ...
      JSON.stringify({
        ...
        files: Object.fromEntries(
          [
            ...
            name,
            { present: true, path: `${rawSentinels.bundlePath}/${name}.json` },
          ]),
        ),
        checks: [
          { name: "release provenance schema is valid", status: "pass" },
          { name: "release approval schema is valid", status: "pass" },
          { name: "airgap redaction passed", status: "pass" },
        ],
        blockers: [],
        redaction: {
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        RELEASE_PROVENANCE_EVIDENCE_PATH: evidencePaths.provenance,
        RELEASE_APPROVAL_EVIDENCE_PATH: evidencePaths.approval,
        RELEASE_PUBLISH_PLAN_PATH: evidencePaths.publishPlan,
        RELEASE_AIRGAP_VERIFICATION_PATH: evidencePaths.airgap,
      }),
    });
    ...
    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.release-security-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.summary).toMatchObject({
      airgapVerified: true,
    ...
    expect(body.data.provenance.sourcePosture).toMatchObject({
      ciRunUrlConfigured: true,
      sourceRefConfigured: true,
      sourceRepoConfigured: true,
    });
    expect(body.data.approval.approval).toMatchObject({
      approverCount: 2,
      minApprovers: 2,
      minApproversSatisfied: true,
      refConfigured: true,
    });
    expect(body.data.publishPlan.steps).toMatchObject({
      gitPush: 1,
      gitTag: 1,
      registryPublish: 2,
    ...
    expect(body.data.airgap.files).toMatchObject({
      approval: true,
      gaBundle: true,
      readbackValidation: true,
    });
    expect(body.data.redaction.commandLinesReturned).toBe(false);
    expect(body.data.redaction.rawEvidencePathsReturned).toBe(false);
    expect(body.data.warnings).toEqual([]);
    expect(serialized).not.toContain(directory);
    for (const value of Object.values(rawSentinels)) {
  ...
  it("reports sanitized release readback posture from mounted files", async () => {
    const directory = mkdtempSync(join(tmpdir(), "romeo-release-readback-"));
    const evidencePaths = {
      plan: join(directory, "release-readback-plan.json"),
      readback: join(directory, "release-readback.json"),
      validation: join(directory, "readback-validation.json"),
    };
    const rawSentinels = {
      assetBaseUrl: "https://releases.example.com/RAW_RELEASE_ASSET_PATH",
      helmRepositoryUrl: "https://charts.example.com/RAW_HELM_REPOSITORY",
      imageRef: "registry.example.com/romeo/app:RAW_IMAGE_TAG",
      packageRegistryUrl: "https://registry.example.com/RAW_PACKAGE_REGISTRY",
      rawHelmBody: "RAW_HELM_REPOSITORY_BODY",
      rawOciManifest: "RAW_OCI_MANIFEST_BODY",
      rawReadbackBody: "RAW_RELEASE_READBACK_BODY",
      rawRegistryResponse: "RAW_REGISTRY_RESPONSE",
      rawTarball: "RAW_PACKAGE_TARBALL_BODY",
      secret: "RAW_RELEASE_SECRET",
      token: "RAW_RELEASE_TOKEN",
    };
    ...
    const assets = releaseAssetNames.map((name, index) => ({
      name,
      sha256: String(index + 2).repeat(64),
    }));
    const chart = { name: "romeo", version: "1.2.3" };
    writeFileSync(
      evidencePaths.plan,
      JSON.stringify({
        schemaVersion: "romeo.release-readback-plan.v1",
        helm: { repositoryUrl: rawSentinels.helmRepositoryUrl },
        images: [
          {
            readback: `${rawSentinels.imageRef}@sha256:${imageDigest}`,
            required: rawSentinels.imageRef,
          },
        ],
        charts: [
          {
            readback: `${chart.name}:${chart.version}@sha256:${chartDigest}`,
            required: `${chart.name}:${chart.version}`,
          },
        ],
        assets: assets.map((asset) => ({
          readback: `${asset.name}=${rawSentinels.assetBaseUrl}/${asset.name}.json@sha256:${asset.sha256}`,
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        registries: {
          npm: {
            registry: rawSentinels.packageRegistryUrl,
            auth: { tokenEnv: "NPM_TOKEN", credentialsUsed: true },
          },
          oci: { auth: { tokenEnv: "OCI_TOKEN", credentialsUsed: true } },
          helm: {
            repository: rawSentinels.helmRepositoryUrl,
            auth: { tokenEnv: "HELM_TOKEN", credentialsUsed: true },
          },
          releaseAssets: {
            auth: { tokenEnv: "ASSET_TOKEN", credentialsUsed: true },
          },
        },
        packages: [
          {
            ...
            version: "1.2.3",
            registry: rawSentinels.packageRegistryUrl,
            tarballBody: rawSentinels.rawTarball,
          },
        ],
        images: [
          {
            image: rawSentinels.imageRef,
            source: "oci_registry",
            manifestBody: rawSentinels.rawOciManifest,
          },
        ],
        helmCharts: [
          {
            ...
            repositoryBody: rawSentinels.rawHelmBody,
          },
        ],
        assets: assets.map((asset) => ({
          name: asset.name,
          source: "release_asset",
          url: `${rawSentinels.assetBaseUrl}/${asset.name}.json`,
          sha256: asset.sha256,
        })),
        rawReadbackBody: rawSentinels.rawReadbackBody,
        rawRegistryResponse: rawSentinels.rawRegistryResponse,
        secretValue: rawSentinels.secret,
      }),
      "utf8",
    );
    writeFileSync(
      evidencePaths.validation,
      JSON.stringify({
        ...
        release: {
          ...
          readbackFile: evidencePaths.readback,
        },
        required: {
          packages: [
            {
          ...
          images: [rawSentinels.imageRef],
          charts: [{ ...chart, sha256: chartDigest }],
          assets,
        },
        checks: [
          { name: "credentialed npm registry readback", status: "pass" },
          {
            name: `${rawSentinels.imageRef} image registry readback is verified`,
            status: "pass",
          },
          {
        ...
        rawValidationBody: rawSentinels.rawReadbackBody,
        token: rawSentinels.token,
      }),
      "utf8",
    );
    ...
    const api = createRomeoApi(new InMemoryRomeoRepository(), {
      env: readEnv({
        RELEASE_READBACK_PLAN_PATH: evidencePaths.plan,
        RELEASE_READBACK_EVIDENCE_PATH: evidencePaths.readback,
        RELEASE_READBACK_VALIDATION_PATH: evidencePaths.validation,
      }),
    });
    ...
    expect(response.status).toBe(200);
    expect(body.data.schema).toBe("romeo.release-readback-posture.v1");
    expect(body.data.status).toBe("ready");
    expect(body.data.summary).toMatchObject({
      planReady: true,
    ...
    expect(body.data.plan).toMatchObject({
      ...
      assets: { total: 5, digestPinned: 5, requiredMatched: 5 },
    });
    expect(body.data.readback).toMatchObject({
      status: "satisfied",
      mode: "live_registry_readback",
    ...
    expect(body.data.warnings).toEqual([]);
    expect(body.data.redaction.rawEvidencePathsReturned).toBe(false);
    expect(body.data.redaction.packageRegistryUrlsReturned).toBe(false);
    expect(body.data.redaction.releaseAssetUrlsReturned).toBe(false);
    expect(body.data.redaction.tokenValuesReturned).toBe(false);
    for (const value of Object.values(evidencePaths)) {
      expect(serialized).not.toContain(value);
    }
    expect(serialized).not.toContain(directory);
