...
describe("Prompt template sharing", () => {
  it("keeps private prompts hidden until shared and exposes marketplace prompts without leaking bodies in audit logs", async () => {
    ...
    const hiddenPage = await service.listPage(viewer, "workspace_default", {
      limit: 1,
      offset: 0,
      query: "incident",
    });
    ...
    const sharedPage = await service.listPage(viewer, "workspace_default", {
      limit: 1,
      offset: 0,
      query: "incident",
    });
    ...
    const marketplace = await service.marketplace(
      viewer,
      "workspace_default",
      "launch",
    );
    ...
    expect(sharedPage).toMatchObject({ total: 1, limit: 1, offset: 0 });
    expect(sharedPage.items.map((prompt) => prompt.id)).toEqual([
      privatePrompt.id,
    ]);
    expect(marketplace.map((prompt) => prompt.id)).toEqual([
      marketplacePrompt.id,
    ]);
    expect(privatePrompt.tags).toEqual(["ops", "ir"]);
    expect(
      auditLogs.some(
        (log) =>
          log.action === "prompt_template.share" &&
          log.resourceId === privatePrompt.id,
      ),
    ).toBe(true);
