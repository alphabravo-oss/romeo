...
describe("Romeo usage summaries", () => {
  it("aggregates run usage by metric, actor, and provider", async () => {
    ...
    expect(exportedCsv).toContain(
      "id,createdAt,actorId,workspaceId,sourceType,sourceId,metric,quantity,unit,providerId,modelId,agentId,estimatedCostUsd",
    );
    ...
    expect(
      summary.totals.some(
        (item) => item.metric === "run.started" && item.quantity === 1,
      ),
    ).toBe(true);
    expect(
      summary.totals.some(
        (item) => item.metric === "run.completed" && item.quantity === 1,
      ),
    ).toBe(true);
    expect(
      summary.totals.some(
    ...
    expect(
      summary.totals.some(
        (item) =>
          item.metric === "llm.input_token.reported" &&
          item.quantity > 0 &&
          item.estimatedCostUsd > 0,
      ),
    ).toBe(true);
    expect(
      summary.totals.some(
        (item) =>
          item.metric === "llm.output_token.reported" &&
          item.quantity > 0 &&
          item.estimatedCostUsd > 0,
      ),
    ).toBe(true);
    expect(
      summary.totals.some(
        (item) =>
          item.metric === "llm.total_token.reported" && item.quantity > 0,
      ),
    ).toBe(true);
    expect(
      summary.byActor.some(
        (item) =>
          item.actorId === "user_dev_admin" && item.metric === "run.started",
      ),
    ).toBe(true);
    expect(
      summary.byProvider.some(
        (item) =>
          item.providerId === "provider_openai_compatible" &&
          item.metric === "run.started",
      ),
    ).toBe(true);
    expect(
      summary.byProvider.some(
...
async function waitForCompletedSummary(api: ReturnType<typeof createRomeoApi>) {
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    ...
    const body = await response.json();
    summary = body.data;
    if (summary.totals.some((item) => item.metric === "run.completed"))
      return summary;
