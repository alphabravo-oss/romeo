...
describe("group administration", () => {
  it("creates groups, manages memberships, exposes durable share targets, and authenticates member group IDs", async () => {
    ...
    const addResponse = await api.request(
      `/api/v1/groups/${group.data.id}/members`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
    ...
    const shareTargetsResponse = await api.request(
    ...
    const removeResponse = await api.request(
      `/api/v1/groups/${group.data.id}/members/user_reviewer`,
      { method: "DELETE" },
    );
    const removed = await removeResponse.json();
    const auditResponse = await api.request(
      "/api/v1/audit-logs?action=group.member.add",
    );
    ...
    expect(shareTargets.data).toContainEqual({
      principalType: "group",
