...
export function buildRunMessages(
  input: BuildRunMessagesInput,
): BuildRunMessagesResult {
  const contextWindow = modelContextWindow(input.model);
  const reserve = Math.min(
    maxOutputReserveTokens,
    Math.floor(contextWindow * outputReserveWindowFraction),
  );
  const usable = Math.floor(
    Math.max(0, contextWindow - reserve) * estimatorSafetyFraction,
  );
  ...
  const candidates =
    input.maxHistoryMessages === undefined
      ? allCandidates
  ...
  let userMessage = knowledgeUserMessage(
    hits,
    input.userContent,
    input.userImages,
  );
  ...
  if (budgetEnabled) {
    while (floorTokens > usable && hits.length > 0) {
      ...
      userMessage = knowledgeUserMessage(
        hits,
        input.userContent,
        input.userImages,
      );
  ...
  const kept = keepNewestWithinBudget(
    candidates,
