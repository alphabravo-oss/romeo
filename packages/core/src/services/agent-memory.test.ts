...
import {
  ...
  historyMessageLimit,
  normalizeAgentMemoryPolicy,
} from "./agent-memory";

describe("normalizeAgentMemoryPolicy", () => {
  it("collapses undefined and disabled to the default policy", () => {
    expect(normalizeAgentMemoryPolicy(undefined)).toEqual(
      defaultAgentMemoryPolicy,
    );
    expect(normalizeAgentMemoryPolicy({ mode: "disabled" })).toEqual(
      defaultAgentMemoryPolicy,
    );
  ...
  it("materializes the default recent message count", () => {
    expect(normalizeAgentMemoryPolicy({ mode: "recent_messages" })).toEqual({
  ...
  it("preserves an in-range explicit count", () => {
    expect(
      normalizeAgentMemoryPolicy({ mode: "recent_messages", maxMessages: 2 }),
    ).toEqual({ mode: "recent_messages", maxMessages: 2 });
  });
...
describe("historyMessageLimit", () => {
  it("returns the message cap for recent_messages", () => {
    expect(
      historyMessageLimit({ mode: "recent_messages", maxMessages: 4 }),
    ).toBe(4);
