...
const WORKFLOW_TEMPLATES: WorkflowTemplateDefinition[] = [
  {
    template: {
      ...
      description:
        "Runs an agent draft step, waits for human approval, then records a notification ledger step.",
      ...
      steps: [
        { type: "agent_run", name: "Draft response", requiresAgentId: true },
        {
          type: "approval",
          name: "Human approval",
          approvalPrompt: "Approve the agent draft before notification.",
        },
        {
          type: "notification",
          name: "Record approval notification",
  ...
  },
  {
    template: {
      ...
      description:
        "Runs an agent step and records a notification ledger step without an approval gate.",
      requiredInputs: ["agentId"],
      steps: [
        { type: "agent_run", name: "Agent run", requiresAgentId: true },
        {
          type: "notification",
          name: "Record completion notification",
...
export function buildWorkflowFromTemplate(input: WorkflowTemplateCreateInput): {
  ...
  const definition = WORKFLOW_TEMPLATES.find(
    (entry) => entry.template.id === input.templateId,
  );
  ...
  if (
    definition.template.requiredInputs.includes("agentId") &&
    input.agentId === undefined
  ) {
    throw new ApiError(
      "invalid_workflow_template_input",
      "This workflow template requires an agentId.",
      400,
    );
  }
  ...
  return {
    ...
    steps: definition.template.steps.map((step) =>
      instantiateStep(step, input.agentId),
    ),
  };
}
...
function instantiateStep(
  ...
  if (step.type === "agent_run" || step.type === "agent_handoff") {
    if (agentId === undefined)
      throw new ApiError(
        "invalid_workflow_template_input",
        "Agent run template steps require an agentId.",
        400,
      );
    return {
      type: step.type,
      name: step.name,
      agentId,
      ...(step.handoffFromStepId === undefined
        ? {}
        : { handoffFromStepId: step.handoffFromStepId }),
  ...
  if (step.type === "approval") {
    return {
      ...
      ...(step.approvalPrompt === undefined
        ? {}
        : { approvalPrompt: step.approvalPrompt }),
    };
  }
  if (step.type === "tool_approval") {
    return {
      type: step.type,
      name: step.name,
      ...(step.toolChainName === undefined
        ? {}
        : { toolChainName: step.toolChainName }),
  ...
  if (step.type === "agent_room") {
    throw new ApiError(
      "invalid_workflow_template_input",
      "Agent room template steps require explicit agent IDs.",
      400,
    );
  }
  return {
