...
const promptInjectionDetectors: Array<{
...
}> = [
  {
    ...
    pattern:
      /\b(ignore|disregard|forget|override|bypass)\b[\s\S]{0,120}\b(previous|prior|above|earlier|system|developer)\b[\s\S]{0,80}\b(instruction|prompt|message|rule)s?\b/i,
  },
  {
    category: "system_prompt_exfiltration",
    pattern:
      /\b(reveal|show|print|dump|expose|return)\b[\s\S]{0,120}\b(system prompt|developer message|hidden instruction|internal prompt|policy)\b/i,
  },
  {
    category: "credential_exfiltration",
    pattern:
      /\b(send|exfiltrate|leak|steal|extract|dump)\b[\s\S]{0,120}\b(api key|token|credential|secret|password|private key)s?\b/i,
  },
  {
    category: "tool_or_policy_bypass",
    pattern:
      /\b(bypass|skip|disable|ignore)\b[\s\S]{0,120}\b(tool approval|approval|permission|authorization|safety|guardrail)s?\b/i,
  },
];

export function normalizeAgentSafetySettings(
  input: AgentSafetySettings | undefined,
): AgentSafetySettings {
  const settings: AgentSafetySettings = {};

  if (input?.maxUserInputLength !== undefined) {
    if (
      !Number.isInteger(input.maxUserInputLength) ||
      input.maxUserInputLength < 1 ||
      input.maxUserInputLength > maxConfiguredInputLength
    ) {
      throw new ApiError(
        "invalid_agent_safety_settings",
        `Max user input length must be between 1 and ${maxConfiguredInputLength}.`,
        400,
      );
    }
  ...
  const promptInjectionGuard = normalizePromptInjectionGuard(
    input?.promptInjectionGuard,
  );
  if (promptInjectionGuard !== undefined)
    settings.promptInjectionGuard = promptInjectionGuard;
...
export function enforceAgentSafetySettings(
  ...
  if (
    ...
    content.length > settings.maxUserInputLength
  ) {
    throw new ApiError(
      "agent_safety_input_too_long",
      "User input exceeds this agent safety limit.",
      400,
      {
  ...
  throw new ApiError(
    ...
    "User input violates this agent prompt-injection policy.",
    400,
    {
...
export function shouldScanPromptInjection(
  settings: AgentSafetySettings,
  source: AgentSafetySource,
): boolean {
  const guard = settings.promptInjectionGuard;
  if (guard === undefined || guard.mode !== "block") return false;
  return source === "retrieved_context"
...
export function inspectPromptInjection(
  ...
  const normalized = content
    .slice(0, maxPromptInjectionScanLength)
    .replace(/\s+/g, " ");
...
function normalizeBlockedTerms(terms: string[] | undefined): string[] {
  ...
  if (terms.length > maxBlockedTerms) {
    throw new ApiError(
      "invalid_agent_safety_settings",
      `Blocked terms supports at most ${maxBlockedTerms} entries.`,
      400,
    );
  }
  ...
  if (normalized.some((term) => term.length > maxBlockedTermLength)) {
    throw new ApiError(
      "invalid_agent_safety_settings",
      `Blocked terms must be ${maxBlockedTermLength} characters or fewer.`,
      400,
    );
  }
...
function normalizePromptInjectionGuard(
  input: AgentSafetySettings["promptInjectionGuard"] | undefined,
): AgentSafetySettings["promptInjectionGuard"] | undefined {
  ...
  if (scanUserInput !== true && scanRetrievedContext !== true) {
    throw new ApiError(
      "invalid_agent_safety_settings",
      "Prompt injection guard must scan user input or retrieved context.",
      400,
    );
  }
