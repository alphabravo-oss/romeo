...
export async function importOpenApiToolConnector(
  ...
  if (operations.length === 0)
    throw new ApiError(
      ...
      "OpenAPI spec must include at least one supported operation.",
      400,
    );
  ...
  const connector: ToolConnector = {
    ...
    approvalPolicy:
      input.approvalPolicy ?? (hasWrite ? "external_side_effects" : "never"),
    visibility: "org",
    enabled: false,
    createdAt: now,
    updatedAt: now,
  ...
  const createdOperations = await repository.createToolOperations(
    operations.map((operation) =>
      toToolOperation(subject, created.id, operation, input, now),
    ),
  );
...
function toToolOperation(
  ...
  return {
    ...
    approvalPolicy:
      input.approvalPolicy ?? (isRead ? "never" : "external_side_effects"),
    enabled: false,
    createdAt: now,
  };
}
...
function readOpenApiAuthHints(
  ...
  for (const [schemeId, scheme] of Object.entries(securitySchemes)) {
    ...
    const clientCredentials = asRecord(
      asRecord(record.flows)?.clientCredentials,
    );
    const tokenUrl = readOpenApiOAuthTokenUrl(
      readString(clientCredentials?.tokenUrl),
      baseUrl,
    );
    if (
      record.type === "oauth2" &&
      ...
      hints.push({
        ...
        oauthScopes: normalizeOAuthScopes(
          Object.keys(asRecord(clientCredentials.scopes) ?? {}),
        ),
      });
    }
  }
...
function readOpenApiOAuthTokenUrl(
  ...
  try {
    const resolved =
      baseUrl === undefined
        ? new URL(value)
        : new URL(value, baseUrl.endsWith("/") ? baseUrl : `${baseUrl}/`);
