...
export async function resolveRunContext(
  ...
  const [chat, agent] = await Promise.all([
    repository.getChat(input.chatId),
  ...
  if (chat.archivedAt !== undefined) {
    throw new ApiError(
      "chat_archived",
      "Archived chats cannot start new runs.",
      409,
    );
  }
  ...
  if (!agent.publishedVersionId) {
    throw new ApiError(
      ...
      "Agent must be published before it can run.",
      409,
    );
  }

  const agentVersion = await repository.getAgentVersion(
    agent.publishedVersionId,
  );
  if (!agentVersion || agentVersion.agentId !== agent.id)
  ...
  if (!model.enabled) {
    throw new ApiError(
      "model_disabled",
      "This model is disabled and cannot be used to start a run.",
      409,
    );
  }
  ...
  if (!provider.enabled) {
    throw new ApiError(
      "provider_disabled",
      "This model provider is disabled and cannot be used to start a run.",
      409,
    );
  }
