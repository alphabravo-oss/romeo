...
export function readOoxmlZipEntries(
  bytes: Uint8Array,
  options: OoxmlZipOptions,
): Map<string, OoxmlZipEntry> {
  if (bytes.byteLength > options.maxBytes) {
    throw new ApiError(
      "extraction_input_too_large",
      "Knowledge source is too large for Office extraction.",
      413,
      {
  ...
  if (
    ...
    throw new ApiError(
      "unsupported_archive",
      "Split ZIP archives are not supported for Office extraction.",
      415,
    );
  }
  ...
  if (centralDirectoryOffset + centralDirectorySize > buffer.byteLength) {
    throw new ApiError(
      "invalid_archive",
      "Office document ZIP central directory is invalid.",
      422,
    );
  }
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (entry.uncompressedSize > options.maxEntryBytes) {
      throw new ApiError(
        ...
        "Office document ZIP entry is too large for extraction.",
        413,
        {
    ...
    if (expandedBytes > options.maxExpandedBytes) {
      throw new ApiError(
        ...
        "Office document expanded size exceeds the extraction safety limit.",
        413,
        {
    ...
    if (compressionRatio > options.maxCompressionRatio) {
      throw new ApiError(
        ...
        "Office document compression ratio exceeds the extraction safety limit.",
        413,
        {
...
function readCentralDirectoryEntry(
  buffer: Buffer,
  offset: number,
): CentralDirectoryEntry & { nextOffset: number } {
  if (
    ...
    throw new ApiError(
      "invalid_archive",
      "Office document ZIP central directory entry is invalid.",
      422,
    );
  }
  ...
  if (nextOffset > buffer.byteLength)
    throw new ApiError(
      "invalid_archive",
      "Office document ZIP entry extends past the archive.",
      422,
    );
  if ((flags & 1) === 1)
    throw new ApiError(
      "encrypted_archive",
      "Encrypted Office documents are not supported for extraction.",
      415,
    );
  if (
    compressedSize === zip64Sentinel ||
    uncompressedSize === zip64Sentinel ||
    localHeaderOffset === zip64Sentinel
  ) {
    throw new ApiError(
      "unsupported_archive",
      "ZIP64 Office documents are not supported for extraction.",
      415,
    );
  }
  if (compressionMethod !== 0 && compressionMethod !== 8) {
    throw new ApiError(
      ...
      "Office document ZIP compression method is not supported.",
      415,
      { compressionMethod },
    );
  }
  ...
  return {
    ...
    read() {
      ...
      if (inflated.byteLength !== entry.uncompressedSize) {
        throw new ApiError(
          "invalid_archive",
          "Office document ZIP entry size did not match its central directory.",
          422,
          { entryName: entry.name },
        );
      }
...
function readCompressedEntry(
  ...
  if (
    ...
    buffer.readUInt32LE(offset) !== localFileHeaderSignature
  ) {
    throw new ApiError(
      "invalid_archive",
      "Office document ZIP local file header is invalid.",
      422,
      { entryName: entry.name },
    );
  }
...
function findEndOfCentralDirectory(buffer: Buffer): number {
  ...
  throw new ApiError(
    "invalid_archive",
    "Office document ZIP end-of-central-directory record was not found.",
    422,
  );
}
