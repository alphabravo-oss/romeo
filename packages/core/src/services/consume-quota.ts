...
export async function consumeQuota(
  ...
  return consumeQuotas(repository, subject, [input], options);
}
...
export async function consumeQuotas(
  ...
  const buckets = await resetDueQuotaBuckets(
    repository,
    await repository.listQuotaBuckets(subject.orgId),
  );
  const matching = inputs.flatMap((input) =>
    buckets
      .filter(
        (bucket) =>
          bucket.metric === input.metric &&
          matchesQuotaScope(bucket, subject, input),
      )
      .map((bucket) => ({ bucket, input })),
  );

  for (const { bucket, input } of matching) {
    if (bucket.used + input.quantity > bucket.limit) {
      throw new ApiError(
        "quota_exceeded",
        `Quota exceeded for ${input.metric}.`,
        429,
        {
          metric: input.metric,
          limit: bucket.limit,
  ...
  const quantities = Object.fromEntries(
    matching.map(({ bucket, input }) => [bucket.id, input.quantity]),
  );
  ...
  const reservation =
    options.quotaCoordinator === undefined
      ...
      : await options.quotaCoordinator.reserve({
          buckets: matching.map(({ bucket }) =>
            toQuotaReservationBucket(bucket),
          ),
          quantity: inputs.length === 1 ? inputs[0]!.quantity : 0,
          ...(inputs.length === 1 ? {} : { quantities }),
        });
  if (reservation?.allowed === false) {
    const matched = matching.find(
      (item) => item.bucket.id === reservation.bucketId,
    );
    throw new ApiError(
      "quota_exceeded",
      `Quota exceeded for ${matched?.input.metric ?? "requested usage"}.`,
      429,
      {
        metric: matched?.input.metric ?? "unknown",
        limit: reservation.limit,
        used: reservation.used,
        requested: matched?.input.quantity ?? 0,
        ...(matched === undefined
          ? {}
          : {
              quotaBucketId: matched.bucket.id,
              scopeType: matched.bucket.scopeType,
            }),
      },
    );
  }
  ...
  const updated = await Promise.all(
    matching.map(async ({ bucket, input }) => ({
      before: bucket,
      after: await repository.updateQuotaBucket({
        ...bucket,
        used: Math.max(
          bucket.used + input.quantity,
          reservedUsage.get(bucket.id) ?? 0,
        ),
  ...
  for (const bucket of updated)
    emitQuotaAlert(subject, bucket.before, bucket.after, options.webhooks);
}
...
function matchesQuotaScope(
  ...
  if (bucket.scopeType === "workspace")
    return bucket.scopeId === input.workspaceId;
  if (bucket.scopeType === "provider")
    return bucket.scopeId === input.providerId;
...
function emitQuotaAlert(
  ...
  if (
    severity === undefined ||
    severityRank(severity) <= severityRank(severityFor(before))
  )
    return;
