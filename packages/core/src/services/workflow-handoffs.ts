...
export async function buildWorkflowHandoffPrompt(input: {
  ...
  if (sourceStep === undefined) {
    throw new ApiError(
      ...
      "Handoff steps require a completed upstream agent step.",
      409,
    );
  }
  ...
  if (sourceChatId === undefined || sourceRunId === undefined) {
    throw new ApiError(
      "workflow_handoff_invalid_state",
      "Handoff source step is missing linked chat or run metadata.",
      409,
    );
  }
  ...
  if (
    ...
    throw new ApiError(
      "workflow_handoff_invalid_state",
      "Handoff source step references an invalid upstream run.",
      409,
    );
  }
  ...
  const instruction =
    input.step.handoffPrompt ??
    workflowStepInstruction(input.workflow, input.step, input.runInput);
  return {
    content: [
      ...
      assistantContext.length > 0
        ...
        : "Prior assistant output was not available.",
    ].join("\n\n"),
    sourceChatId,
    sourceRunId,
...
function resolveHandoffSourceStep(
  step: WorkflowStep,
  completedSteps: WorkflowStepRun[],
): WorkflowStepRun | undefined {
  const candidates = completedSteps.filter((candidate) =>
    isCompletedAgentStep(candidate),
  );
  if (step.handoffFromStepId !== undefined)
    return candidates.find(
      (candidate) => candidate.stepId === step.handoffFromStepId,
    );
...
function isCompletedAgentStep(step: WorkflowStepRun): boolean {
  return (
    ...
    (step.type === "agent_run" || step.type === "agent_handoff")
  );
}

function boundedLatestAssistantContent(
  messages: Awaited<ReturnType<RomeoRepository["listMessages"]>>,
): string {
  ...
  return content.length <= MAX_HANDOFF_CONTEXT_CHARACTERS
    ? content
    : content.slice(-MAX_HANDOFF_CONTEXT_CHARACTERS);
}

function workflowStepInstruction(
  workflow: WorkflowDefinition,
  step: WorkflowStep,
  runInput: Record<string, unknown>,
): string {
  return (
    stringInput(runInput.prompt) ??
    stringInput(runInput.content) ??
    `Continue workflow "${workflow.name}" step "${step.name}".`
  );
}
