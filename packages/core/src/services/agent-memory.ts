...
export function normalizeAgentMemoryPolicy(
  input: AgentMemoryPolicy | undefined,
): AgentMemoryPolicy {
  if (input === undefined || input.mode === "disabled")
    return defaultAgentMemoryPolicy;
  if (input.mode !== "recent_messages")
    throw new ApiError(
      "invalid_agent_memory_policy",
      "Unsupported agent memory policy mode.",
      400,
    );
  ...
  if (
    ...
    maxMessages < 1 ||
    maxMessages > maxRecentMessageCount
  ) {
    throw new ApiError(
      "invalid_agent_memory_policy",
      `Recent message memory must include between 1 and ${maxRecentMessageCount} messages.`,
      400,
    );
  }
...
export function historyMessageLimit(
  ...
  return normalized.mode === "recent_messages"
