...
export function stripeBillingWebhookEvent(
  input: StripeBillingWebhookInput,
): ExternalBillingEventInput {
  return stripeEventToBillingEvent(constructStripeEvent(input));
}

export function genericBillingWebhookEvent(
  input: GenericBillingWebhookInput,
): ExternalBillingEventInput {
...
function verifyGenericSignature(input: GenericBillingWebhookInput): void {
  if (input.secret.trim().length === 0)
    throw new ApiError(
      ...
      "Generic billing webhooks are not configured.",
      503,
    );
  ...
  if (
    ...
    timestamp <= 0 ||
    Math.abs(nowSeconds - timestamp) > input.toleranceSeconds
  ) {
    throw new ApiError(
...
function parseGenericSignature(header: string | undefined): string {
  if (header === undefined || header.trim().length === 0) {
    throw new ApiError(
      ...
      "The billing webhook signature header is required.",
      401,
    );
  }
  const value = header.trim().startsWith("v1=")
    ? header.trim().slice("v1=".length)
    : header.trim();
  if (!/^[a-f0-9]{64}$/i.test(value))
    throw new ApiError(
      "billing_webhook_signature_invalid",
      "The billing webhook signature header is invalid.",
      401,
    );
  return value;
}

function constructStripeEvent(input: StripeBillingWebhookInput): Stripe.Event {
  if (input.secret.trim().length === 0)
    throw new ApiError(
      "billing_webhook_not_configured",
      "Stripe billing webhooks are not configured.",
      503,
    );
  if (
...
function stripeEventToBillingEvent(
  event: Stripe.Event,
): ExternalBillingEventInput {
  const type = event.type;
  const object = parseJsonRecord(
    event.data.object,
    "Stripe billing webhook object is invalid.",
  );
  ...
  return {
    ...
    externalInvoiceId: eventType.startsWith("invoice.")
      ? stringValue(object.id)
      : undefined,
    invoiceStatus: eventType.startsWith("invoice.")
      ? boundedString(object.status, 80)
      : undefined,
    amountCents: eventType.startsWith("invoice.")
      ? invoiceAmountCents(object)
      : undefined,
...
function genericPayloadToBillingEvent(
  payload: string,
): ExternalBillingEventInput {
  const event = parseJsonRecord(
    payload,
...
function genericEventType(
  value: unknown,
): ExternalBillingEventInput["eventType"] {
  if (
  ...
  throw new ApiError(
    ...
    "The billing webhook event type is not supported.",
    400,
  );
}
...
function stripeEventType(
  ...
  throw new ApiError(
    "billing_webhook_event_unsupported",
    "The Stripe billing webhook event type is not supported.",
    400,
  );
}
...
function subscriptionStatus(
  eventType: ExternalBillingEventInput["eventType"],
  object: Record<string, unknown>,
): ExternalBillingEventInput["status"] | undefined {
...
function stripeLifecycle(
  object: Record<string, unknown>,
): ExternalBillingEventInput["lifecycle"] | undefined {
...
function boundedString(value: unknown, max: number): string | undefined {
  ...
  if (text === undefined || text.length === 0 || text.length > max)
    return undefined;
  return text;
}
...
function compactLifecycle(
  value: NonNullable<ExternalBillingEventInput["lifecycle"]>,
): ExternalBillingEventInput["lifecycle"] | undefined {
...
function quotaTemplates(
  value: unknown,
): ExternalBillingEventInput["quotaTemplates"] | undefined {
  ...
  const templates = value
    ...
    .filter(
      (
        ...
        ExternalBillingEventInput["quotaTemplates"]
...
function quotaTemplate(
  ...
  | NonNullable<ExternalBillingEventInput["quotaTemplates"]>[number]
  | undefined {
