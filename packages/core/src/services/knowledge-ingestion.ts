...
export function extractKnowledgeSourceBytes(
  bytes: Uint8Array,
  mimeType: string,
...
export function extractInlineKnowledgeContent(
  content: string,
  mimeType: string,
): ExtractedKnowledgeText {
  return extractKnowledgeSourceBytes(
    new TextEncoder().encode(content),
    mimeType,
  );
}

export function createChunksForSource(
  ...
  return chunks.map((chunk, index) => ({
    ...
    metadata: {
      ...
      embedding: createTextEmbedding(chunk),
      ...metadata,
    },
...
export function toIndexedKnowledgeChunks(
  chunks: KnowledgeChunk[],
  sources: KnowledgeSource[],
): IndexedChunk[] {
  ...
  return chunks.map((chunk): IndexedChunk => {
    ...
    const withEmbedding =
      embedding === undefined ? indexedChunk : { ...indexedChunk, embedding };
    return source?.objectKey
      ? { ...withEmbedding, sourceUri: source.objectKey }
      : withEmbedding;
  });
}

export function retrievalHitFromIndexedChunk(
  chunk: IndexedChunk,
  score: number,
): RetrievalHit {
  const citation = {
