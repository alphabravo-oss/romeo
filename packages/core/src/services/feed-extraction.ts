...
export function extractFeedText(
  xml: string,
  maxItemsValue: unknown,
): FeedExtractionResult {
  const maxItems = normalizeMaxItems(maxItemsValue);
  const blocks = extractElementBlocks(xml, ["item", "entry"]).slice(
    0,
    maxItems,
  );
  const lines: string[] = [];
  const feedTitle = firstText(
    firstElementBlock(xml, ["channel", "feed"]) ?? xml,
    ["title"],
  );
  ...
  for (const item of blocks.map(parseFeedItem)) {
    ...
    if (item.published !== undefined)
      lines.push(`Published: ${markdownLine(item.published)}`);
    if (item.author !== undefined)
      lines.push(`Author: ${markdownLine(item.author)}`);
    if (item.categories.length > 0)
      lines.push(`Categories: ${item.categories.map(markdownLine).join(", ")}`);
    if (item.link !== undefined) lines.push(`Link: ${markdownLine(item.link)}`);
    for (const enclosure of item.enclosures)
      lines.push(`Enclosure: ${markdownLine(enclosure)}`);
  ...
  if (content.length === 0)
    throw new ApiError(
      ...
      "RSS connector feed did not contain indexable text.",
      422,
    );
...
function parseFeedItem(block: string): FeedItem {
  const link =
    rssLink(block) ?? atomAlternateLink(block) ?? guidPermalink(block);
  const published = firstText(block, [
    "pubDate",
    "published",
    "updated",
    "date",
  ...
  return {
    ...
    body:
      firstText(block, ["encoded", "content", "description", "summary"]) ?? "",
    ...(link !== undefined ? { link } : {}),
...
function allText(xml: string, localName: string): string[] {
  ...
  return [...xml.matchAll(pattern)]
    .map((match) => xmlToPlainText(match[2] ?? ""))
    .filter((value) => value.length > 0);
}

function firstLocalNameText(
  xml: string,
  localName: string,
): string | undefined {
...
function atomAlternateLink(block: string): string | undefined {
  const links = startTagAttributes(block, "link").filter(
    (attrs) => attrs.href !== undefined,
  );
  const alternate = links.find(
    (attrs) => attrs.rel === undefined || attrs.rel === "alternate",
  );
...
function formatEnclosure(
  ...
  const details = [
    type,
    length === undefined ? undefined : `${length} bytes`,
