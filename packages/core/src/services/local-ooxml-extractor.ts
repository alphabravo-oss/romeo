...
const defaultMaxExpandedBytes = 50_000_000;
  ...
  async extract(input: {
    ...
    if (kind === undefined) {
      throw new ApiError(
        ...
        "Local Office extraction only supports docx, pptx, and xlsx sources.",
        415,
        { mimeType },
      );
    }
    ...
    const entries = readOoxmlZipEntries(input.bytes, {
      ...
      maxCompressionRatio:
        this.options.maxCompressionRatio ?? defaultMaxCompressionRatio,
      maxEntries: this.options.maxEntries ?? defaultMaxEntries,
      maxEntryBytes: this.options.maxEntryBytes ?? defaultMaxEntryBytes,
      maxExpandedBytes:
        this.options.maxExpandedBytes ?? defaultMaxExpandedBytes,
    });
    const parts =
      kind === "docx"
        ? extractDocxParts(entries)
        : kind === "pptx"
          ? extractPptxParts(entries)
          : extractXlsxParts(entries);
...
function extractXlsxParts(entries: Map<string, OoxmlZipEntry>): string[] {
  ...
  return [...entries.keys()]
    ...
    .sort((left, right) => naturalCompare(left, right))
    .map((name) =>
      extractWorksheetText(readEntryText(entries.get(name)), sharedStrings),
    )
    .filter((part) => part.length > 0);
}

function readXmlParts(
  entries: Map<string, OoxmlZipEntry>,
  names: string[],
): string[] {
  return names
    .map((name) => xmlText(readEntryText(entries.get(name))))
    .filter((part) => part.length > 0);
}
  ...
  for (const match of xml.matchAll(/<c\b([^>]*)>([\s\S]*?)<\/c>/g)) {
    ...
    if (type === "s") {
      ...
      const value = decodeXmlEntities(
        readFirstElementText(cellXml, "v"),
      ).trim();
      if (value.length > 0) parts.push(value);
    }
  }
...
function requireContent(content: string): string {
  if (content.length === 0)
    throw new ApiError(
      "empty_extraction",
      "Office document extraction produced no indexable text.",
      422,
    );
  return content;
}
