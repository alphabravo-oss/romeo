...
describe("LocalOoxmlTextExtractor", () => {
  ...
  it("extracts PPTX slide and speaker-note text in natural slide order", async () => {
    ...
    const bytes = createZip([
      {
        ...
        content: "<p:sld><a:t>Final rollout checkpoint</a:t></p:sld>",
      },
      {
        name: "ppt/slides/slide2.xml",
        content: "<p:sld><a:t>Phase two controls</a:t></p:sld>",
      },
      {
        name: "ppt/notesSlides/notesSlide2.xml",
        content: "<p:notes><a:t>Speaker note evidence</a:t></p:notes>",
      },
    ]);
    ...
    expect(result.content.indexOf("Phase two controls")).toBeLessThan(
      result.content.indexOf("Final rollout checkpoint"),
    );
    ...
    const bytes = createZip([
      {
        ...
        content:
          "<sst><si><t>Security review</t></si><si><r><t>Retention</t></r><r><t> evidence</t></r></si></sst>",
      },
      {
  ...
  it("rejects oversized Office XML parts before indexing content", async () => {
    ...
    const bytes = createZip([
      {
        name: "word/document.xml",
        content: "<w:t>This part is too large.</w:t>",
      },
    ]);
  ...
  it("rejects Office archive entries with a decompression-bomb ratio", async () => {
    const extractor = new LocalOoxmlTextExtractor({ maxCompressionRatio: 5 });
    const bytes = createZip([
      {
        name: "word/document.xml",
        content: `<w:t>${"A".repeat(10_000)}</w:t>`,
      },
    ]);
  ...
  it("rejects aggregate expanded Office content before inflating entries", async () => {
    const extractor = new LocalOoxmlTextExtractor({
      maxCompressionRatio: 10_000,
      maxExpandedBytes: 50,
    });
    const bytes = createZip([
      { name: "word/document.xml", content: `<w:t>${"A".repeat(30)}</w:t>` },
      { name: "word/header1.xml", content: `<w:t>${"B".repeat(30)}</w:t>` },
    ]);
