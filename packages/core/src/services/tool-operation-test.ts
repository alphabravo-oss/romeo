...
export function buildToolOperationTestPreview(
  ...
  const disabledReasons = disabledExecutionReasons(
    connector,
    operation,
    options,
  );
  ...
  return {
    ...
    requestPreview: {
      ...
      networkExecution:
        ...
        options.externalExecutionEnabled === true
          ? "worker_ready"
          : "disabled",
    },
  };
...
function buildExecutionPlan(
  ...
  const secretRefScheme =
    secretRef === undefined ? undefined : safeSecretRefScheme(secretRef);
...
function disabledExecutionReasons(
  connector: ToolConnector,
  operation: ToolOperation,
  options: ToolOperationTestOptions,
): ToolOperationTestDisabledReason[] {
  ...
  if (requiresAuth(connector) && connector.authConfig.configured !== true)
    reasons.push("auth_not_configured");
  if (
    connector.networkPolicy.mode !== "allow_hosts" ||
    connector.networkPolicy.allowedHosts.length === 0
  )
    reasons.push("network_policy_missing");
  if (options.externalExecutionEnabled !== true)
    reasons.push("external_execution_disabled");
  return reasons;
}
...
function hasConnectorBaseUrl(connector: ToolConnector): boolean {
  return (
    ...
    connector.schema.baseUrl.length > 0
  );
}
