  ...
  async extract(input: {
    ...
    if (mimeType !== "application/pdf") {
      throw new ApiError(
        ...
        "Local PDF extraction only supports application/pdf sources.",
        415,
        { mimeType },
      );
    }
    if (!hasPdfHeader(input.bytes)) {
      throw new ApiError(
        "invalid_pdf_header",
        "PDF source does not start with a valid PDF header.",
        422,
      );
    }
    ...
    if (input.bytes.byteLength > maxBytes) {
      throw new ApiError(
        ...
        "Knowledge source is too large for local PDF extraction.",
        413,
        {
          maxBytes,
    ...
    try {
      ...
      const result = await runner(
        this.options.commandPath ?? "pdftotext",
...
function requireContent(content: string): string {
  if (content.length === 0)
    throw new ApiError(
      "empty_extraction",
      "PDF text extraction produced no indexable text.",
      422,
    );
  return content;
}
