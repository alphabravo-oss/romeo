...
describe("feed extraction", () => {
  it("extracts RSS item metadata and sanitizes HTML-rich bodies", () => {
    ...
    expect(result.content).toContain(
      "Link: https://docs.example.com/releases/connectors?ref=rss&lang=en",
    );
    expect(result.content).toContain(
      "Enclosure: https://docs.example.com/audio.mp3 (audio/mpeg, 1200 bytes)",
    );
    expect(result.content).toContain(
      "Romeo RSS connector sync imports richer feed entries.",
    );
  ...
  it("extracts Atom links, authors, categories, and HTML content", () => {
    ...
    expect(result.content).toContain(
      "Link: https://docs.example.com/blog/atom-connectors",
    );
    expect(result.content).toContain(
      "Enclosure: https://docs.example.com/blog/atom-connectors.pdf (application/pdf, 2048 bytes)",
    );
    expect(result.content).toContain(
      "Atom feed extraction keeps connector knowledge sync useful.",
    );
  });
  ...
  it("bounds extracted entries and rejects empty feeds", () => {
    ...
    expect(() => extractFeedText("<rss><channel></channel></rss>", 10)).toThrow(
      ApiError,
    );
  });
});
