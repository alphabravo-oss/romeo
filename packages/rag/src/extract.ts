...
export function canExtractKnowledgeText(mimeType: string): boolean {
  ...
  return (
    normalized.startsWith("text/") || structuredTextMimeTypes.has(normalized)
  );
}

export function extractKnowledgeText(
  input: ExtractKnowledgeTextInput,
  ...
  if (!canExtractKnowledgeText(mimeType)) {
    throw new KnowledgeExtractionError(
      "unsupported_media_type",
      "Knowledge extraction is only available for supported textual formats.",
      { mimeType },
    );
  }
  if (input.bytes.byteLength > maxBytes) {
    throw new KnowledgeExtractionError(
      "extraction_input_too_large",
      "Knowledge source is too large for synchronous text extraction.",
      {
        maxBytes,
...
function decodeUtf8(bytes: Uint8Array): string {
  try {
    ...
    throw new KnowledgeExtractionError(
      "unsupported_encoding",
      "Knowledge source text must be UTF-8 encoded.",
    );
  }
}
...
function normalizeJson(text: string, mimeType: string): ExtractedKnowledgeText {
  try {
    ...
    throw new KnowledgeExtractionError(
      "invalid_json",
      "JSON knowledge source could not be parsed.",
      { mimeType },
    );
  }
}

function normalizeNdjson(
  text: string,
  mimeType: string,
): ExtractedKnowledgeText {
  const lines = text
  ...
  try {
    ...
    throw new KnowledgeExtractionError(
      "invalid_json",
      "NDJSON knowledge source could not be parsed.",
      { mimeType },
    );
  }
}
...
function requireContent(text: string): string {
  ...
  if (content.length === 0)
    throw new KnowledgeExtractionError(
      "empty_extraction",
      "Knowledge extraction produced no indexable text.",
    );
  return content;
}
