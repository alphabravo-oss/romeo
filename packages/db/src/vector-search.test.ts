...
import {
  toKnowledgeChunkEmbeddingInsertValue,
  toKnowledgeChunkEmbeddingRecord,
  type KnowledgeChunkEmbeddingRecord,
...
describe("pgvector search helpers", () => {
  ...
  it("rejects dimension mismatches before SQL construction", () => {
    expect(() => toPgVectorLiteral([0, 1], 3)).toThrow(
      "Expected 3 vector dimensions",
    );
  });

  it("rejects non-finite vector values before SQL construction", () => {
    expect(() => toPgVectorLiteral([0, Number.NaN], 2)).toThrow(
      "finite numbers",
    );
    expect(() => toPgVectorLiteral([0, Number.POSITIVE_INFINITY], 2)).toThrow(
      "finite numbers",
    );
  });
