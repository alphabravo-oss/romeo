...
describe("OIDC JWT verification", () => {
  ...
  it("enforces the configured algorithm and required expiration claim", async () => {
    const keys = await createRsaKeyPair("kid_1");
    const wrongAlgorithm = await signJwt(
      keys.privateKey,
      { alg: "PS256", kid: "kid_1" },
      validClaims(),
    );
    const missingExpiration = await signJwt(
      keys.privateKey,
      { alg: "RS256", kid: "kid_1" },
    ...
    await expect(
      verifyOidcJwt(missingExpiration, {
        issuer: "https://idp.example.com/realms/romeo",
        audience: "romeo",
        jwks: [keys.publicJwk],
      }),
    ).rejects.toThrow("include exp");
  });
});
