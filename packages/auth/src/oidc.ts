...
export function mapOidcClaimsToSubject(
  claims: Record<string, unknown>,
  config: OidcClaimMappingConfig,
): OidcMappedSubject {
  const subject = stringClaim(claims.sub);
  if (subject === undefined)
    throw new OidcClaimMappingError(
      "OIDC claims must include a string sub claim.",
    );

  const groupOptions: { clientId?: string; groupClaim: string } = {
  ...
  const mappedSubject: OidcMappedSubject = {
    ...
    orgId: config.orgId,
    workspaceIds: dedupe([
      ...config.defaultWorkspaceIds,
      ...mappedWorkspaceIds(externalGroups, config),
    ]),
...
function mappedWorkspaceIds(
  ...
  if (
    config.workspaceGroupPrefix !== undefined &&
    config.workspaceGroupPrefix.length > 0
  ) {
    for (const group of groups) {
      if (
        group.startsWith(config.workspaceGroupPrefix) &&
        group.length > config.workspaceGroupPrefix.length
      ) {
