...
export function validateToolDispatchResponse(input: {
  body: Uint8Array;
  contentType?: string;
  ...
  const schema =
    input.responseValidation?.jsonSchemas[String(input.status)] ??
    input.responseValidation?.jsonSchemas.default;
  if (schema === undefined) return { status: "not_applicable" };
  if (input.truncated)
    return { status: "skipped", errorCode: "response_body_truncated" };
  if (input.contentType === undefined || !isJsonContentType(input.contentType))
    return {
      status: "skipped",
      errorCode: "response_content_type_unsupported",
    };
...
function validateJsonSchemaSubset(
  ...
  if (
    ...
    !enumValues.some((item) => JSON.stringify(item) === JSON.stringify(value))
  ) {
    return "response_enum_mismatch";
  }
  ...
  if (type === "object") {
    ...
    if (required.some((key) => !(key in record)))
      return "response_required_property_missing";
    const properties = asRecord(schema.properties);
    if (properties !== undefined) {
      for (const [key, childSchema] of Object.entries(properties)) {
        if (!(key in record)) continue;
        const errorCode = validateJsonSchemaSubset(
          record[key],
          asRecord(childSchema) ?? {},
          depth + 1,
        );
...
function schemaType(schema: Record<string, unknown>): string | undefined {
  ...
  if (Array.isArray(schema.type))
    return schema.type.find((item): item is string => typeof item === "string");
