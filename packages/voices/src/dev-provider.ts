...
export class DevVoiceProvider implements VoiceProvider {
  ...
  async synthesize(request: SpeechRequest): Promise<SpeechSynthesisArtifact> {
    ...
    const body =
      request.format === "wav" ? createWavTone(durationMs) : undefined;
    return {
      id,
      contentType:
        request.format === "mp3" ? "audio/mpeg" : `audio/${request.format}`,
...
function createWavTone(durationMs: number): Uint8Array {
  ...
  for (let index = 0; index < samples; index += 1) {
    ...
    const sample =
      Math.sin((2 * Math.PI * 440 * index) / sampleRate) * 2400 * fade;
