...
describe("OpenAI-compatible voice provider", () => {
  it("sends bounded speech requests and returns audio bytes without exposing the API key", async () => {
    ...
    const artifact = await provider.synthesize({
      orgId: "org_default",
      voiceId: "alloy",
      text: "Romeo voice preview",
      format: "wav",
    });

    const speechCall = calls.find((call) => call.url.endsWith("/audio/speech"));
    expect(speechCall?.url).toBe("https://voice.example.com/v1/audio/speech");
    const speechHeaders = new Headers(speechCall?.headers);
    expect(speechHeaders.get("authorization")).toBe("Bearer voice-api-key");
    expect(speechHeaders.get("content-type")).toBe("application/json");
    expect(JSON.parse(String(speechCall?.body))).toEqual({
      model: "tts-model",
      voice: "alloy",
  ...
  it("sends transcription requests as multipart form data", async () => {
    ...
    const transcriptionCall = calls.find((call) =>
      call.url.endsWith("/audio/transcriptions"),
    );
    const body = transcriptionCall?.body;

    expect(transcriptionCall?.url).toBe(
      "https://voice.example.com/v1/audio/transcriptions",
    );
    expect(new Headers(transcriptionCall?.headers).get("authorization")).toBe(
      "Bearer voice-api-key",
    );
  ...
  it("parses a compact configured voice catalog", async () => {
    const voices = parseOpenAICompatibleVoiceCatalog(
      "alloy=Alloy, echo=Echo, bad/path=Bad",
    );
    const provider = new OpenAICompatibleVoiceProvider({
      apiKey: "voice-api-key",
      baseUrl: "https://voice.example.com/v1",
