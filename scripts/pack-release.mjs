...
const apiClientTarball = join(
  outDir,
  `romeo-api-client-${apiClientPackage.version}.tgz`,
);
...
assertTarball(cliTarball, {
  required: [
    "package/package.json",
    "package/bin/romeo.mjs",
    "package/src/index.ts",
    "package/src/knowledge-worker.ts",
    "package/src/webhook-worker.ts",
  ],
  forbidden: [".test.ts"],
});

const packedCliPackage = JSON.parse(
  capture("tar", ["-xOf", cliTarball, "package/package.json"]),
);
if (
  packedCliPackage.dependencies?.["@romeo/api-client"] !==
  apiClientPackage.version
) {
  throw new Error(
    "Packed CLI dependency does not match the packed API client version.",
  );
}
...
function assertTarball(path, checks) {
  ...
  for (const forbidden of checks.forbidden) {
    ...
    if (match !== undefined)
      throw new Error(`Forbidden artifact ${match} found in ${path}`);
  }
}
