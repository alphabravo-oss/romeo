...
const outputPath = repoPath(
  argValue("--output") ?? "dist/release/upgrade-validation.json",
);
...
const plan = {
  ...
  targetVersion,
  release:
    targetRelease === undefined ? undefined : releaseSummary(targetRelease),
  checks,
};
...
if (!stdout)
  console.log(
    `Wrote Romeo upgrade validation to ${relative(root, outputPath)}`,
  );
...
function validateVersionProgression() {
  ...
  check(
    "target version is not older than current version",
    compareVersions(target, from) >= 0,
  );
}

function validateReleaseTimestamps(release) {
  check(
    "channel generatedAt is a valid timestamp",
    validTimestamp(channel.generatedAt),
  );
  check(
    "release generatedAt is a valid timestamp",
    validTimestamp(release.generatedAt),
  );
}
...
function validateCompatibility(release) {
  ...
  check(
    "Node compatibility metadata is present",
    typeof nodeRange === "string" && nodeRange.length > 0,
  );
  check(
    "pnpm compatibility metadata is present",
    typeof pnpmRange === "string" && pnpmRange.length > 0,
  );
  if (typeof nodeRange === "string") {
    check(
      "current Node satisfies release minimum",
      satisfiesMinimum(process.versions.node, nodeRange),
    );
  }
}
...
function validatePublishOrder(release) {
  ...
  check(
    "SDK publishes before CLI",
    apiClientIndex >= 0 && cliIndex >= 0 && apiClientIndex < cliIndex,
  );
}
...
function validateOptionalEvidence(release) {
  ...
  if (requireProvenance) {
    check(
      ...
      typeof release.provenanceUrl === "string" &&
        release.provenanceUrl.startsWith("https://"),
    );
  }
  if (
    requireUpgradeGuide &&
    compareVersions(
      ...
      parseVersion(fromVersion, "--from-version"),
    ) > 0
  ) {
    check(
      "upgrade guide URL is present",
      typeof release.upgradeGuideUrl === "string" &&
        release.upgradeGuideUrl.startsWith("https://"),
    );
  }
}
...
function satisfiesMinimum(version, range) {
  ...
  return (
    compareVersions(
      parseVersion(version, "current Node version"),
      parseVersion(match[1], "Node range"),
    ) >= 0
  );
}
