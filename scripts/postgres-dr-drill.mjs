...
const databaseUrl = nonEmpty(
  argValue("--database-url") ?? process.env.DRILL_DATABASE_URL,
);
if (databaseUrl === undefined)
  throw new Error(
    "DRILL_DATABASE_URL or --database-url is required for restore drills.",
  );
...
const output =
  outputValue === undefined
    ? repoPath(`backups/romeo-dr-drill-${timestampForFilename()}.json`)
    : resolveRepoPath(outputValue);
const expectedSha256 = argValue("--expected-sha256");
const downloadUrl = nonEmpty(
  argValue("--download-url") ?? process.env.POSTGRES_RESTORE_DOWNLOAD_URL,
);
...
if (expectedSha256 !== undefined)
  restoreArgs.push("--expected-sha256", expectedSha256);
  ...
  printPlan({
    ...
    download:
      downloadUrl === undefined
        ? undefined
        : { type: "presigned_get", url: redactedRemoteUrl(downloadUrl) },
...
if (!confirmed) {
  throw new Error(
    "Restore drills are destructive. Re-run with --confirm-isolated-target after selecting an isolated target database.",
  );
}
...
const result = spawnSync(process.execPath, restoreArgs, {
  ...
  env: {
    ...
    ...(downloadUrl === undefined
      ? {}
      : { POSTGRES_RESTORE_DOWNLOAD_URL: downloadUrl }),
  },
  stdio: "inherit",
});
