...
const graph = JSON.parse(
  execFileSync(
    "pnpm",
    ["list", "-r", "--json", "--prod", "--depth", "Infinity"],
    { encoding: "utf8" },
  ),
);
...
for (const project of graph) {
  ...
  if (typeof project.name !== "string" || projectVersion === undefined)
    continue;
...
const bom = {
  ...
  metadata: {
    timestamp,
    tools: [
      { vendor: "Romeo", name: "generate-sbom", version: rootPackage.version },
    ],
    component: {
      type: "application",
...
if (stdout) {
  ...
  console.log(
    `Wrote CycloneDX SBOM with ${bom.components.length} components to ${output}`,
  );
}
  ...
  if (typeof node.path === "string") {
    try {
      const manifest = JSON.parse(
        readFileSync(`${node.path}/package.json`, "utf8"),
      );
...
function externalReferencesFor(input) {
  const refs = [];
  if (
    typeof input.resolved === "string" &&
    input.resolved.startsWith("https://")
  ) {
  ...
  if (typeof input.path === "string" && input.path.startsWith(process.cwd())) {
    refs.push({
      type: "other",
      url: `workspace:${relative(process.cwd(), input.path) || "."}`,
    });
  }
  return refs;
}
...
function deterministicSerialNumber(components, timestampValue) {
  const hash = createHash("sha256")
    .update(
      JSON.stringify({
        components: components.map((component) => component["bom-ref"]).sort(),
