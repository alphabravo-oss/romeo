...
if (!dryRun && !hasFlag("--confirm")) {
  throw new Error(
    "PostgreSQL restore is destructive. Re-run with --confirm after validating the target database and backup file.",
  );
}
...
async function downloadBackup(url, path) {
  ...
  if (!response.ok || response.body === null)
    throw new Error(`Backup download failed with HTTP ${response.status}.`);
  ensureParentDirectory(path);
  await pipeline(Readable.fromWeb(response.body), createWriteStream(path));
  console.log(
    `Downloaded PostgreSQL backup from ${redactedRemoteUrl(url)} to ${path}`,
  );
}
